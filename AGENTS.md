...
- `core/domain/` — business rules: `entities/` (session), `policies/` (catalog, models_catalog, permission, role), `defaults/` (config, prompts, providers, linters, skills).
- `core/application/` — use-cases: `generation/`, `permission/`, `provider/`, `session/`, `rules/`, `skills/`, `linters/`.
...
- `core/adapters/` — external LLM providers (anthropic, gemini, openai). `core/base_provider/` holds provider internals (agent, compaction, errors, tools).
- `core/roles/` — assistant role registry/apply/resolve/prompt/provider/tools.
