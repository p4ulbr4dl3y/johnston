...
from johnston_tui.presentation.screens.rewind import (
    RewindScreen,
    RewindSelection,
    format_rewind_files,
)
...
class TestRewindActionScreen(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_open_diff_viewer(self):
        ...
        async with app.run_test() as pilot:
            ...
            with patch("johnston_core.domain.ports.checkpoint.get_checkpoint_manager", return_value=mock_cm), \
    ...
    async def test_open_diff_viewer_empty_text_and_no_cm(self):
        ...
        async with app.run_test() as pilot:
            ...
            with patch("johnston_core.domain.ports.checkpoint.get_checkpoint_manager", side_effect=Exception("no cm")), \
                 patch.object(app, "push_screen") as mock_push:
                screen._open_diff_viewer()
                mock_push.assert_called_once()
                diff_screen = mock_push.call_args[0][0]
...
class TestRewindScreen(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_refresh_options_responsive_width_breakpoint(self):
        ...
        async with app.run_test() as pilot:
            ...
            with patch("johnston_tui.utils.responsive.resolve_screen_width", return_value=30):
                screen._refresh_options()
...
class TestProvidersScreen(unittest.IsolatedAsyncioTestCase):
    ...
    def test_get_provider_model_count_sources(self):
        ...
        with patch("os.path.exists", return_value=True), \
             patch("johnston_tui.presentation.screens.providers.cached_json_read", return_value={"models": ["m1", "m2", "m3"]}):
            self.assertEqual(screen._get_provider_model_count("cached_p", {}), 3)

        # 3. From models catalog
        with patch("os.path.exists", return_value=False), \
             patch("johnston_tui.presentation.screens.providers.catalog.get_catalog_provider", return_value={"models": ["cat1"]}):
            self.assertEqual(screen._get_provider_model_count("catalog_p", {}), 1)

        # 4. Fallback 0
        with patch("os.path.exists", return_value=False), \
             patch("johnston_tui.presentation.screens.providers.catalog.get_catalog_provider", side_effect=Exception("error")):
    ...
    async def test_toggle_disabled_action_and_reconciliation(self):
        ...
        async with app.run_test() as pilot:
            ...
            with patch("johnston_tui.app.role_service.reconcile_active_agent") as mock_reconcile:
...
class TestSkillsAndMCPScreenHint(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_skills_screen_hint_right_text(self):
        ...
        with patch("johnston_tui.presentation.screens.skills.get_skill_manager") as mock_get_sm:
            mock_sm = MagicMock()
    ...
    async def test_mcp_screen_hint_right_text(self):
        ...
        with patch("johnston_tui.presentation.screens.mcp.get_mcp_manager") as mock_get_mm:
            mock_mm = MagicMock()
