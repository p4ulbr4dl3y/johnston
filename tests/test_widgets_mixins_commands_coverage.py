...
from johnston_tui.chat_input import DEFAULT_PLACEHOLDER
from johnston_tui.git_metrics_mixin import GitMetricsMixin
...
async def test_theme_command_execution_and_callbacks():
    ...
    from johnston_tui.app.theme_manager import theme_manager
...
async def test_session_persistence_load_ui_full_flow():
    ...
    with patch(
        "johnston_tui.presentation.widgets.chat_container.restore_message_item",
...
async def test_session_persistence_load_ui_recompute_context_tokens_and_call_after_refresh():
    ...
    with patch("johnston_tui.app.session_state.recompute_context_tokens", return_value=42) as mock_recompute:
        with patch("johnston_tui.presentation.widgets.chat_container.restore_message_item", new_callable=AsyncMock):
...
def test_lifecycle_on_mount_normal_flow():
    ...
    with patch("johnston_tui.mixins.lifecycle.install_asyncio_exception_handler") as mock_install:
        with patch("johnston_core.models_catalog.catalog.load_cache") as mock_cache:
            with patch("johnston_core.infrastructure.mcp.get_mcp_manager") as mock_mcp:
                mcp_mock = MagicMock()
...
def test_lifecycle_on_mount_resume_session_empty_picker():
    ...
    with patch("johnston_tui.presentation.commands.ResumeCommand.execute") as mock_resume_exec:
        with patch("johnston_tui.mixins.lifecycle.install_asyncio_exception_handler"):
            with patch("johnston_core.models_catalog.catalog.load_cache"):
                with patch("johnston_core.infrastructure.mcp.get_mcp_manager"):
                    app.on_mount()
...
async def test_lifecycle_check_initial_setup_variations():
    ...
    with patch.dict(os.environ, {}, clear=True):
        ...
        with patch("johnston_tui.presentation.commands.ProvidersCommand.execute", new_callable=AsyncMock) as mock_prov_cmd:
            await app._check_initial_setup()
        ...
        app.agent.model = ""
        with patch("johnston_tui.presentation.commands.ModelsCommand.execute", new_callable=AsyncMock) as mock_models_cmd:
            await app._check_initial_setup()
...
def test_lifecycle_on_unmount_full_flow():
    ...
    with patch("johnston_tui.app.theme_manager.theme_manager.remove_listener") as mock_rm_listener:
        with patch("johnston_core.application.session.stream.cancel_running_subagents") as mock_subagents:
            with patch.object(app, "save_current_session") as mock_save:
                with patch("johnston_core.infrastructure.mcp.get_mcp_manager") as mock_mcp:
                    with patch("johnston_core.models_catalog.catalog.close", new_callable=AsyncMock):
                        with patch("johnston_core.tools.registry.aclose_tools", return_value=AsyncMock()()):
                            app.on_unmount()
...
def test_git_metrics_compute_branch_sync_variants():
    ...
    with patch("johnston_core.application.generation.prompt_builder.get_git_info", return_value="detached HEAD (1234567)"):
    ...
    with patch("johnston_core.application.generation.prompt_builder.get_git_info", side_effect=Exception("git error")):
...
async def test_session_persistence_edge_cases():
    ...
    with patch("johnston_tui.app.session_state.collect_session_data", return_value={"test": 123}) as mock_collect:
...
def test_lifecycle_on_unmount_sync_and_error_handling():
    ...
    with patch("asyncio.get_running_loop", side_effect=RuntimeError("no loop")):
        with patch.object(app, "_kill_all_tasks_sync") as mock_kill_sync:
            with patch("johnston_tui.app.theme_manager.theme_manager.remove_listener", side_effect=RuntimeError("theme err")):
                with patch("johnston_core.application.session.stream.cancel_running_subagents", side_effect=RuntimeError("subagent err")):
                    with patch.object(app, "save_current_session", side_effect=RuntimeError("save err")):
                        with patch("johnston_core.infrastructure.mcp.get_mcp_manager", side_effect=RuntimeError("mcp err")):
                            with patch("johnston_core.models_catalog.catalog.close", new_callable=AsyncMock):
                                with patch("johnston_core.tools.registry.aclose_tools") as mock_aclose:
...
def test_lifecycle_on_unmount_tools_close_scheduling_failure():
    ...
    with patch("asyncio.get_running_loop", side_effect=RuntimeError("no loop")):
        with patch("johnston_tui.mixins.lifecycle._close_tools_sync", side_effect=RuntimeError("close error")) as mock_close:
            app.on_unmount()
