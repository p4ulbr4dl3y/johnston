...
from johnston_tui.app.status_state import (
    _ensure_cache,
    build_status_kwargs,
...
class TestStatusState(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_collect_cache_load_providers_raises(self):
        from johnston_tui.app import status_state as ss

        pm = MagicMock()
        pm.load_providers.side_effect = Exception("boom")
        app = MagicMock(pm=pm)
        with patch("johnston_core.application.skills.manager.SkillManager.list_skills"), patch(
            "johnston_core.infrastructure.mcp.get_mcp_manager"
        ):
    ...
    async def test_collect_cache_skills_raises(self):
        ...
        with patch(
            "johnston_core.application.skills.manager.SkillManager.list_skills", side_effect=Exception("boom")
    ...
    async def test_collect_cache_mcp_raises(self):
        ...
        with patch("johnston_core.infrastructure.mcp.get_mcp_manager", side_effect=Exception("boom")):
    ...
    async def test_build_status_kwargs_no_widget_loads_providers_and_mcp(self):
        app = self._status_app()
        with patch("johnston_tui.app.status_state.get_mcp_manager") as gm:
            mgr = MagicMock()
