...
from johnston_core.infrastructure.config.settings import JohnstonSettings, UISettings
...
from johnston_tui.presentation.widgets.chat_messages import BotMessage, ThinkingWidget
...
def test_chat_input_max_prompt_history_respects_settings(monkeypatch):
    ...
    monkeypatch.setattr("johnston_tui.chat_input.get_settings", lambda: settings)
...
def test_chat_view_page_size_respects_settings(monkeypatch):
    ...
    monkeypatch.setattr("johnston_tui.presentation.widgets.chat_container.get_settings", lambda: settings)
...
def test_bot_message_stream_flush_interval_respects_settings(monkeypatch):
    ...
    monkeypatch.setattr("johnston_tui.presentation.widgets.chat_messages.get_settings", lambda: settings)
    captured = {}
...
def test_tool_call_widget_stream_flush_interval_respects_settings(monkeypatch):
    settings = JohnstonSettings(ui=UISettings(stream_flush_interval=0.077))
    monkeypatch.setattr("johnston_tui.chat_toolcall.get_settings", lambda: settings)
    captured = {}
