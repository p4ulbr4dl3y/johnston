...
from johnston_tui import chat_input as chat_input_mod
from johnston_tui.chat_input import ChatInput, ClipboardAttachment
...
class TestChatInputUnit(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self):
        ...
        self.patcher = patch("johnston_core.infrastructure.platform.paths.PROMPT_HISTORY_FILE", self.tmp_file)
        self.patcher.start()
    ...
    def test_placeholder_responsiveness(self):
        from johnston_tui.chat_input import (
            COMPACT_PLACEHOLDER,
            DEFAULT_PLACEHOLDER,
            FORK_PLACEHOLDER,
            NARROW_PLACEHOLDER,
    ...
    async def test_try_paste_clipboard_image_non_rgb_mode(self):
        ...
        async with app.run_test():
            ...
            with (
                patch("johnston_core.infrastructure.platform.platform_utils.get_clipboard_image_or_file", return_value=(None, mock_img)),
                patch("os.makedirs"),
    ...
    async def test_prompt_history_persistence(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch("johnston_core.infrastructure.platform.paths.PROMPT_HISTORY_FILE", tmp_history_file), patch("johnston_core.infrastructure.platform.paths.CONFIG_DIR", tmpdir):
                ci = ChatInput()
    ...
    async def test_escape_cancels_workers_without_cancelling_subagents(self):
        ...
        async with app.run_test():
            ...
            with patch.object(App, "workers", new_callable=PropertyMock, return_value=[mock_worker]):
                with patch(
                    "johnston_core.application.session.stream.cancel_running_subagents",
                ) as mock_cancel:
                    event = Key("escape", "escape")
                    event.prevent_default = MagicMock()
                    event.stop = MagicMock()
    ...
    async def test_escape_does_not_cancel_subagents_without_workers(self):
        ...
        app.sm = MagicMock()
        async with app.run_test():
            with patch("johnston_core.application.session.stream.cancel_running_subagents") as mock_cancel:
...
class TestChatInputClipboard(unittest.IsolatedAsyncioTestCase):
    async def test_try_paste_clipboard_image_file_path(self):
        ...
        with patch("johnston_core.infrastructure.platform.platform_utils.get_clipboard_image_or_file", return_value=("/tmp/sample.png", None)):
    ...
    async def test_try_paste_clipboard_image_none(self):
        chat_input = ChatInput()
        with patch("johnston_core.infrastructure.platform.platform_utils.get_clipboard_image_or_file", return_value=(None, None)):
            res = await chat_input.try_paste_clipboard_image()
...
class TestChatInputHistoryFile(unittest.TestCase):
    def test_load_prompt_history_invalid_json_returns_empty(self):
        ...
        with patch.object(chat_input_mod.config, "PROMPT_HISTORY_FILE", "/nonexistent/history.json"):
            with patch("johnston_tui.chat_input.read_json", return_value=None):
                self.assertEqual(ci.load_prompt_history(), [])

    def test_save_prompt_history_swallows_write_error(self):
        ci = ChatInput()
        ci.prompt_history = ["a", "b"]
        with patch("johnston_tui.chat_input.atomic_write_json", side_effect=OSError("disk full")):
...
class TestChatInputScheduleSuggestions(unittest.TestCase):
    ...
    def test_schedule_runtime_error_swallowed(self):
        ...
        with patch.object(type(ci), "is_mounted", new_callable=PropertyMock, return_value=True):
            with patch("johnston_tui.chat_input.asyncio.get_running_loop", side_effect=RuntimeError("no loop")):
...
class TestChatInputHelpAndShellMode(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_typing_bang_enters_shell_mode(self):
        from johnston_tui.chat_input import DEFAULT_SHELL_PLACEHOLDER
        ci, ctx = _app_context()
        await ctx.__aenter__()
        self.addAsyncCleanup(ctx.__aexit__, None, None, None)
