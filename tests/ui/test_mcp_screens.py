...
class TestMCPScreenCoverage(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_on_mount_focus_exception(self):
        ...
        with patch("johnston.tui.presentation.screens.mcp.asyncio.create_task", return_value=MagicMock()):
            screen.on_mount()
...
class TestMCPScreenExtra(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_do_toggle_cancelled_reraises(self):
        ...
        host = _MCPToggleHost()
        async with host.run_test():
            with patch("johnston.tui.presentation.screens.mcp.asyncio.to_thread", side_effect=asyncio.CancelledError()):
    ...
    def test_on_key_tab_toggles_and_space_does_not_crash(self):
        screen = MCPScreen.__new__(MCPScreen)
        screen._toggle_highlighted = MagicMock()
        screen._handle_search_navigation = MagicMock()

        event_tab = MagicMock()
        event_tab.key = "tab"
        screen._on_key(event_tab)
        screen._toggle_highlighted.assert_called_once()
        event_tab.prevent_default.assert_called_once()
        event_tab.stop.assert_called_once()

        event_space = MagicMock()
        event_space.key = "space"
        screen._on_key(event_space)
        screen._handle_search_navigation.assert_called_once_with(event_space)
