...
class TestCommandSuggestionsCoverage(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_long_command_desc_truncated(self):
        sugg = CommandSuggestions()
        cmds = [("/test", "x" * 200)]
        with patch("johnston_tui.command_suggestions.get_all_command_suggestions", new=AsyncMock(return_value=cmds)):
    ...
    async def test_command_desc_expands_on_wide_viewport(self):
        ...
        cmds = [("/test", "x" * 120)]
        with (
            patch("johnston_tui.command_suggestions.get_all_command_suggestions", new=AsyncMock(return_value=cmds)),
            patch("johnston_tui.command_suggestions.resolve_width", return_value=160),
        ):
