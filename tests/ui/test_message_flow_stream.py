...
class TestGenerateNotConnected(unittest.IsolatedAsyncioTestCase):
    async def test_not_connected_runs_providers_command(self):
        ...
        async with app.run_test() as pilot:
            ...
            app.pm.get_active_provider_key = MagicMock(return_value="openai")
            with patch("johnston_tui.presentation.commands.ProvidersCommand", return_value=MagicMock()) as mock_cls:
    ...
    async def test_connected_no_model_runs_models_command(self):
        ...
        async with app.run_test() as pilot:
            ...
            app.agent = MagicMock()
            app.agent.model = ""
            with patch("johnston_tui.presentation.commands.ModelsCommand", return_value=MagicMock()) as mock_cls:
                mock_cls.return_value.execute = unittest.mock.AsyncMock()
                app.generate_ai_response("hello")
                deadline = asyncio.get_running_loop().time() + 10
                while asyncio.get_running_loop().time() < deadline:
...
class TestGenerateStreamEvents(unittest.IsolatedAsyncioTestCase):
    async def _run(self, stream_fn, setup=None):
        app = JohnstonApp()
        with patch("johnston_core.infrastructure.storage.git_checkpoint.GitCheckpointManager.create_checkpoint"):
            async with app.run_test() as pilot:
                await pilot.pause(0.1)
    ...
    async def test_checkpoint_exception_prints(self):
        ...
        app = JohnstonApp()
        with patch("johnston_core.infrastructure.storage.git_checkpoint.GitCheckpointManager.create_checkpoint", side_effect=Exception("boom")):
            async with app.run_test() as pilot:
                await pilot.pause(0.1)
                _configure_connected(app, stream)
        ...
        with patch("johnston_core.infrastructure.storage.git_checkpoint.GitCheckpointManager.create_checkpoint"):
            async with app.run_test() as pilot:
                ...
                while asyncio.get_running_loop().time() < deadline:
                    ...
                    await pilot.pause(0.1)
                self.assertTrue(app.is_generating)
                with patch("johnston_core.infrastructure.runtime.token_util.estimate_tokens", side_effect=Exception("boom")):
                    chat_input = app.query_one("#message-input")
                    chat_input.focus()
                    await pilot.press("escape")
                    await pilot.pause(0.5)
