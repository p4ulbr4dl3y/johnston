...
from johnston.tui.presentation.screens.diff import (
    DiffFooter,
    DiffHeader,
    DiffScreen,
    format_relative_path,
)
from johnston.tui.presentation.screens.help import (
    COMMANDS_DATA,
    KEYBINDINGS_DATA,
    HelpScreen,
    _build_help_table,
...
from johnston.tui.presentation.screens.permission_confirm import (
    PermissionConfirmScreen,
    RejectReasonInput,
)
from johnston.tui.presentation.screens.tasks import (
    BaseTasksListScreen,
    ShellTasksScreen,
    SubagentsScreen,
    TaskConsoleScreen,
...
class TestDiffScreenCoverage(unittest.IsolatedAsyncioTestCase):
    ...
    def test_diff_header_variants(self):
        ...
        with patch("johnston.tui.presentation.screens.diff.resolve_width", return_value=40):
            header.render_header()

        # Test compact width between 52 and 80
        with patch("johnston.tui.presentation.screens.diff.resolve_width", return_value=60):
            header.render_header()

        # Test non-compact width with from_rewind=True
        header_rewind = DiffHeader("Title", "+1 / -1", from_rewind=True)
        with patch("johnston.tui.presentation.screens.diff.resolve_width", return_value=100):
    ...
    def test_diff_footer_variants(self):
        ...
        with patch("johnston.tui.presentation.screens.diff.resolve_width", return_value=90):
            footer.render_footer()

        # File selected, non-compact, width < BREAKPOINT_HINT (e.g. 70)
        with patch("johnston.tui.presentation.screens.diff.resolve_width", return_value=70):
            footer.render_footer()

        # Compact width < 52, diff view
        footer.set_view_context(is_compact=True, compact_view="diff")
        with patch("johnston.tui.presentation.screens.diff.resolve_width", return_value=45):
            footer.render_footer()
    ...
    def test_diff_screen_sidebar_row_width(self):
        ...
        # Compact mode
        with patch("johnston.tui.presentation.screens.diff.resolve_width", return_value=50):
    ...
    def test_diff_screen_render_current_diff_error_fallback(self):
        ...
        with patch("johnston.tui.presentation.screens.diff.format_edit_diff", side_effect=Exception("lexer fail")):
...
class TestHelpScreenCoverage(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_help_screen_pilot_interactions(self):
        ...
        async with HostModalApp(screen).run_test(size=(80, 24)) as pilot:
            ...
            with patch("johnston.tui.utils.responsive.resolve_screen_width", return_value=40):
...
class TestTasksScreensCoverage(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_subagents_screen_pilot(self):
        ...
        with patch("johnston.core.infrastructure.storage.session_store.get_session_store", return_value=mock_store), \
             patch("johnston.tui.presentation.tool_display.is_subagent_running", side_effect=lambda s: s.id in ("sub_1", "sub_3")):
...
class TestAdditionalCoverageEdgeCases(unittest.IsolatedAsyncioTestCase):
    def test_diff_footer_breakpoint_hint_boundary(self):
        ...
        with patch("johnston.tui.presentation.screens.diff.resolve_width", return_value=75):
            footer.render_footer()
