...
from johnston.tui.presentation.tool_display import (
...
class TestSubagentProgressDisplay(unittest.TestCase):
    ...
    def test_format_active_tool_generic(self):
        self.assertEqual(_format_active_tool_progress("search", {"pattern": "foo"}), "searching codebase")
        turn_evts = [
            {"type": "tool", "tool_type": "search", "args": {"pattern": "a"}},
            {"type": "tool", "tool_type": "search", "args": {"pattern": "b"}},
        ]
        self.assertEqual(_format_active_tool_progress("search", {"pattern": "b"}, turn_events=turn_evts), "searching codebase (2)")
...
class TestShellTaskProgressDisplay(unittest.TestCase):
    ...
    def test_subagents_screen_title_hierarchy(self):
        ...
        with patch.object(SubagentsScreen, "app", new_callable=PropertyMock, return_value=mock_app), \
             patch("johnston.core.infrastructure.storage.session_store.get_session_store", return_value=mock_store):
