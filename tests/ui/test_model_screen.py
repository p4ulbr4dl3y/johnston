...
class TestModelScreenBuildData(unittest.TestCase):
    ...
    def test_is_active_model_display_name_match(self):
        with patch("core.domain.policies.models_catalog.catalog.get_model_display_name", return_value="display name"):
            self.assertTrue(ModelScreen._is_active_model("p1", "model-a", "p1", "Model A"))

    def test_is_active_model_display_name_no_match(self):
        with patch("core.domain.policies.models_catalog.catalog.get_model_display_name", side_effect=["foo", "bar"]):
    ...
    def test_build_data_vision_badge(self):
        with patch("core.domain.policies.models_catalog.catalog.has_vision", side_effect=lambda prov, m: m == "gpt-4o"):
