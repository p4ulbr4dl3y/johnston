...
class TestStatusFooter(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_refresh_footer_clean_model_placeholder(self):
        ...
        async with app.run_test() as pilot:
            footer = app.query_one(StatusFooter)
            with patch("johnston.core.domain.policies.models_catalog.catalog.get_model_display_name", return_value=""):
                footer.refresh_footer()
                await pilot.pause()
    ...
    async def test_git_branch_preserves_last_known_on_async_refresh(self):
        ...
        with patch("johnston.core.application.generation.prompt_builder.get_git_info", return_value=""):
    ...
    def test_compute_branch_sync_detached_head(self):
        footer = StatusFooter()
        with patch("johnston.core.application.generation.prompt_builder.get_git_info", return_value="detached HEAD (abc1234)"):
    ...
    async def test_attachment_bar_updates(self):
        from johnston.tui.presentation.widgets.attachment_bar import AttachmentBar, AttachmentChip, AttachmentHint
