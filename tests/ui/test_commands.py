...
from johnston_tui.app.dispatch import COMMAND_REGISTRY, handle_slash_command
from johnston_tui.presentation.commands import (
    BaseCommand,
    CompactCommand,
    ModelsCommand,
    ProvidersCommand,
...
class TestCommands(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_rewind_command_truncates_store_transcript(self):
        ...
        session.messages = [
            ...
            {"type": "user", "text": '<notification type="shell" id="1" title="sh" status="completed">bg shell done</notification>', "show_in_ui": False},
    ...
    async def test_compact_cancelled_by_new_command_clears_task(self):
        ...
        from johnston_tui.presentation.commands.session_commands import NewCommand
        ...
        with patch(
            "johnston_tui.presentation.commands.session_commands.new_session", new=_fake_new_session_compact_wait
    ...
    async def test_compact_cancellation_noop_after_finish(self):
        ...
        from johnston_tui.presentation.commands.helpers import cancel_active_workers
    ...
    async def test_multiple_skills_command(self):
        ...
        with patch("johnston_core.application.skills.manager.SkillManager.get_skill", autospec=True, side_effect=lambda s, n, *a, **kw: {"foo": skill_a, "bar": skill_b}.get(n) or orig_get_skill(s, n, *a, **kw)):
    ...
    async def test_alias_suggestions_formatting(self):
        from johnston_tui.app.command_provider import get_all_command_suggestions
    ...
    async def test_new_command_clears_background_tasks(self):
        ...
        from johnston_core.infrastructure.tasks.shell_task import ShellTask
        from johnston_tui.presentation.commands import NewCommand
    ...
    async def test_subagents_command_no_subagents(self):
        ...
        from johnston_tui.presentation.commands import SubagentsCommand
    ...
    async def test_shell_command_no_tasks(self):
        ...
        from johnston_tui.presentation.commands import ShellTasksCommand
    ...
    async def test_resume_command_clears_queue_and_resets_generating(self):
        ...
        from johnston_tui.presentation.commands import ResumeCommand
    ...
    async def test_mcp_command_pushes_screen(self):
        ...
        from johnston_tui.presentation.commands import MCPCommand

        app = MockApp()
        app.push_screen = MagicMock()

        cmd = MCPCommand()
        with patch("johnston_core.infrastructure.mcp.MCPManager.load_servers", return_value=[{"name": "srv"}]):
            await cmd.execute(app)
    ...
    async def test_mcp_command_no_servers(self):
        ...
        cmd = MCPCommand()
        with patch("johnston_core.infrastructure.mcp.MCPManager.load_servers", return_value=[]):
            await cmd.execute(app)
...
class TestHandleSlashCommand(unittest.IsolatedAsyncioTestCase):
    async def _call(self, text, app=None):
        ...
        from johnston_tui.app.dispatch import handle_slash_command
    ...
    async def test_registered_command_executes_with_args(self):
        app = MagicMock()
        with patch("johnston_tui.app.dispatch.COMMAND_REGISTRY", {"/known": MagicMock()}) as registry:
            inst = AsyncMock()
    ...
    async def test_multiple_whitespace_between_args(self):
        ...
        with patch("johnston_tui.app.dispatch.COMMAND_REGISTRY", {}):
...
class TestCommandsCoverage(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_new_command_cancels_running_worker(self):
        ...
        with patch(
            "johnston_tui.presentation.commands.session_commands.new_session", new=fake_new_session
        ), patch("johnston_core.application.session.stream.cancel_running_subagents"):
            await NewCommand().execute(app)
    ...
    async def test_providers_command_entered_key_opens_with_focus(self):
        ...
        app.push_screen = push_screen
        with patch("johnston_tui.presentation.commands.provider_commands.set_provider_credentials", return_value=""):
            await ProvidersCommand().execute(app)
    ...
    async def test_models_command_disconnected_opens_providers(self):
        app = SimpleApp()
        app.pm = MagicMock()
        with patch("johnston_tui.presentation.commands.provider_commands.fetch_grouped_models", return_value=({}, True)), patch.object(
    ...
    async def test_models_command_no_models_notify(self):
        app = SimpleApp()
        app.pm = MagicMock()
        with patch("johnston_tui.presentation.commands.provider_commands.fetch_grouped_models", return_value=({}, False)), patch.object(
            app, "push_screen"
        ) as ps:
            await ModelsCommand().execute(app)
        ps.assert_not_called()
    ...
    async def test_models_command_provider_model_and_scalar_selection(self):
        ...
        app.push_screen = push_screen
        with patch(
            "johnston_tui.presentation.commands.provider_commands.fetch_grouped_models", return_value=({"p": {"name": "P"}}, False)
        ), patch("johnston_tui.presentation.commands.provider_commands.select_model", side_effect=lambda *a, **k: selected.append(a)):
            await ModelsCommand().execute(app)
    ...
    async def test_thinking_effort_empty_selection_focuses(self):
        ...
        app.push_screen = push_screen
        with patch("johnston_tui.presentation.commands.provider_commands.get_current_thinking_effort", return_value=("p", "m", "auto")):
            await ThinkingEffortCommand().execute(app)
    ...
    async def test_rewind_exception_paths(self):
        from johnston_core.application.session import stream
    ...
    async def test_resume_teardown_scoped_to_old_session_subagents(self):
        ...
        with patch("johnston_tui.presentation.commands.session_commands.cancel_active_workers_and_tasks") as m_cancel:
            await ResumeCommand().execute(app)
    ...
    async def test_skills_no_skills(self):
        app = SimpleApp()
        with patch("johnston_core.application.skills.manager.SkillManager.list_skills", return_value=[]), patch.object(
            app, "push_screen"
        ) as ps:
    ...
    async def test_skills_with_selection(self):
        ...
        with patch("johnston_core.application.skills.manager.SkillManager.list_skills", new=fake_list_skills):
            await SkillsCommand().execute(app)
        self.assertTrue(selected)
    ...
    async def test_compact_divider_raises_and_save_raises(self):
        ...
        with patch("johnston_tui.presentation.commands.session_commands.compact_session", new=fake_compact), patch.object(
            app, "push_screen", MagicMock()
        ):
            await CompactCommand().execute(app)
    ...
    async def test_compact_success_with_queued_message(self):
        ...
        with patch("johnston_tui.presentation.commands.session_commands.compact_session", return_value=outcome):
            await CompactCommand().execute(app)
        for _ in range(5):
    ...
    async def test_compact_success_saves_event_divider(self):
        ...
        with patch("johnston_tui.presentation.commands.session_commands.compact_session", return_value=outcome) as mock_compact:
            await CompactCommand().execute(app)
    ...
    async def test_sandbox_command_toggles_state(self):
        from johnston_tui.presentation.commands import SandboxCommand
        ...
        cmd = SandboxCommand()
        with patch("johnston_core.infrastructure.config.config_helpers.save_sandbox_config") as mock_save:
            await cmd.execute(app)
    ...
    async def test_copy_command_success(self):
        from johnston_tui.presentation.commands import CopyCommand
...
class TestDiffCommand(unittest.IsolatedAsyncioTestCase):
    async def test_diff_command_no_session(self):
        from johnston_tui.presentation.commands.session_commands import DiffCommand
    ...
    async def test_diff_command_scoped_files_pushed(self):
        ...
        with patch("johnston_core.application.session.actions.get_session_diff", new_callable=AsyncMock) as mock_diff:
