...
class TestSubagentViewScreenPilot(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_subagent_screen_allows_selection(self):
        ...
        async with app.run_test() as pilot:
            await pilot.pause(0.2)
            from johnston_tui.presentation.widgets.chat_messages import BotMessage, UserMessage
    ...
    def test_subagent_status_footer_token_cache(self):
        ...
        with patch("johnston_core.infrastructure.runtime.token_util.estimate_tokens", return_value=42) as mock_est:
    ...
    async def test_subagent_screen_preserves_plan_state(self):
        ...
        sess.add_event({
            ...
            "tool_type": "update_plan",
            "args": {
                "plan": [{"step": "Step A", "status": "in_progress"}],
                "explanation": "Expl",
            },
        })
        screen = SubagentViewScreen("task-plan-persist")
        app = DummyHostApp(screen, store=self.store)
        ...
        app2 = DummyHostApp(screen2, store=self.store)
        app2._subagent_plan_state = {"task-plan-persist": plan_state}
    ...
    async def test_subagent_kill_drops_late_listener_events_from_chat_view(self):
        ...
        from johnston_tui.presentation.widgets.chat_messages import EventDivider
    ...
    async def test_session_chat_screen_alias_and_init(self):
        from johnston_tui.presentation.screens.subagent_screen import SessionChatScreen, SubagentViewScreen
