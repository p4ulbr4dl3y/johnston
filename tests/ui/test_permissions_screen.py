...
class TestPermissionsScreenPilot(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_sections_builtin_mcp_and_shell_guard_standalone(self):
        ...
        self.assertEqual(sg["label"], "ShellGuard")
        # ShellGuard is standalone: it must be the last item, outside any group
        self.assertEqual(items[-1]["type"], "shell_guard")
    ...
    async def test_toggle_mcp_tool_uses_exposed_name(self):
        ...
        with patch("core.mcp_manager.get_mcp_manager", return_value=mgr):
            ...
            async with DummyHostApp(screen).run_test() as pilot:
                ...
                mock_upd.assert_called_once_with("tool", "srv__search", "deny")
