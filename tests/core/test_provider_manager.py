...
def pm(tmp_path, monkeypatch):
    monkeypatch.setattr("johnston.core.application.provider.provider_manager.CONFIG_DIR", str(tmp_path))
    monkeypatch.setattr("johnston.core.application.provider.provider_manager.CONFIG_FILE", str(tmp_path / "config.json"))
    monkeypatch.setattr("johnston.core.application.provider.provider_manager.PROVIDERS_JSON_FILE", str(tmp_path / "providers.json"))
    monkeypatch.setattr("johnston.core.application.provider.provider_manager.CACHE_DIR", str(tmp_path / "cache"))
...
class TestProviderManager(unittest.TestCase):
    def setUp(self):
        ...
        self.config_dir_patcher = patch("johnston.core.application.provider.provider_manager.CONFIG_DIR", self.test_dir)
        self.config_file_patcher = patch(
            "johnston.core.application.provider.provider_manager.CONFIG_FILE", os.path.join(self.test_dir, "config.json")
        )
        self.providers_json_patcher = patch(
            "johnston.core.application.provider.provider_manager.PROVIDERS_JSON_FILE", os.path.join(self.test_dir, "providers.json")
        )
        self.cache_dir_patcher = patch("johnston.core.application.provider.provider_manager.CACHE_DIR", os.path.join(self.test_dir, "cache"))
...
class TestProviderManagerJson(unittest.TestCase):
    def test_json_providers_loading(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with (
                patch("johnston.core.application.provider.provider_manager.PROVIDERS_JSON_FILE", json_file),
                patch("johnston.core.application.provider.provider_manager.CONFIG_FILE", config_file),
                patch("johnston.core.application.provider.provider_manager.CONFIG_DIR", tmpdir),
            ):
                pm = ProviderManager()
...
def test_unicode_and_quoted_key_values(pm):
    ...
    from johnston.core.infrastructure.secrets import load_secrets
...
def test_connected_env_key(pm, monkeypatch):
    ...
    from johnston.core.infrastructure.secrets import save_secret
...
def test_env_api_key_togetherai(pm, monkeypatch):
    ...
    from johnston.core.infrastructure.secrets import get_secret
...
def test_select_model_switching_provider_sets_live_agent_model(pm, tmp_path):
    ...
    from johnston.core.application.provider.actions import select_model
...
def test_is_local_provider():
    from johnston.core.application.provider.provider_manager import is_local_provider
