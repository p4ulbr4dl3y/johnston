...
class _PM:
    ...
    def __enter__(self):
        self.patcher = patch("johnston.core.application.permission.permission_manager.CONFIG_FILE", self.cfg)
        self.patcher.start()
        return self.pm
...
@pytest.fixture
def store(tmp_path):
    """A SessionStore rooted in tmp_path (WORKSPACES_DIR patched)."""
