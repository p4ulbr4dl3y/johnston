...
def test_delete_reload_from_fresh_store(store):
    ...
    with patch("johnston_core.infrastructure.storage.session_store.PROJECTS_DIR", projects_dir):
...
def test_sanitize_session_event():
    from johnston_core.domain.entities.session import sanitize_session_event
...
def test_serialize_session_jsonl_corrupted_fallback(store):
    from johnston_core.infrastructure.storage.session_store import _serialize_session_jsonl
