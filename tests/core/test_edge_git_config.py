...
read/write path (johnston.core.infrastructure.platform.platform_utils.read_json / atomic_write_json,
johnston.core.infrastructure.config.config_helpers.ensure_json_config, ProviderManager._read_config).
...
from johnston.core.infrastructure.config.config_helpers import ensure_json_config
from johnston.core.infrastructure.platform.platform_utils import atomic_write_json, read_json
from johnston.core.infrastructure.runtime.git_utils import make_git_diff, run_git
...
def test_returns_124_timeout_when_git_hangs():
    ...
    with patch(
        "johnston.core.infrastructure.runtime.git_utils.subprocess.run",
...
def test_timeout_none_passes_through_and_completes():
    ...
    with patch("johnston.core.infrastructure.runtime.git_utils.subprocess.run") as m:
...
def test_git_missing_in_path_returns_1():
    with patch("johnston.core.infrastructure.runtime.git_utils.subprocess.run", side_effect=FileNotFoundError("No such file: git")):
        res = run_git(["status"])
    assert res.returncode == 1
...
def test_provider_manager_read_config_non_dict_returns_empty(tmp_path):
    ...
    with patch("johnston.core.application.provider.provider_manager.CONFIG_FILE", str(tmp_path / "cfg.json")):
        with patch("johnston.core.application.provider.provider_manager.PROVIDERS_JSON_FILE", str(tmp_path / "prov.json")):
...
def test_provider_manager_missing_config_returns_empty(tmp_path):
    with patch("johnston.core.application.provider.provider_manager.CONFIG_FILE", str(tmp_path / "absent.json")):
        with patch("johnston.core.application.provider.provider_manager.PROVIDERS_JSON_FILE", str(tmp_path / "prov.json")):
            pm = ProviderManager()
            assert pm._read_config() == {}
            assert pm._get_config_data() == {}
