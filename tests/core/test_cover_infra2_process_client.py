...
import johnston_core.infrastructure.mcp.process_client as core_infra_process_client
...
class TestCoverReaderMisc:
    ...
    def test_join_stderr_thread_alive_logs(self):
        ...
        thread.is_alive.return_value = True
        client._stderr_thread = thread
        with patch("johnston_core.infrastructure.mcp.process_client.logger") as mock_logger:
...
class TestCoverPopenKwargs:
    def test_build_popen_kwargs_win32_creationflags(self):
        ...
        with (
            patch("johnston_core.infrastructure.mcp.process_client.sys.platform", "win32"),
            patch.object(
...
class TestCoverTerminate:
    ...
    def test_terminate_posix_killpg_and_wait_fail(self):
        ...
        with (
            patch("johnston_core.infrastructure.mcp.process_client.sys.platform", "linux"),
            patch(
                "johnston_core.infrastructure.mcp.process_client.os.killpg", side_effect=[None, OSError("kill fail")]
            ) as killpg,
        ):
    ...
    def test_terminate_win32_kill_and_wait_fail(self):
        ...
        with patch("johnston_core.infrastructure.mcp.process_client.sys.platform", "win32"):
            client._terminate_process_group()
    ...
    def test_terminate_stream_none_skipped(self):
        ...
        with (
            patch("johnston_core.infrastructure.mcp.process_client.sys.platform", "linux"),
            patch("johnston_core.infrastructure.mcp.process_client.os.killpg"),
        ):
            client._terminate_process_group()
...
class TestCoverFetchAndStale:
    ...
    def test_is_tools_stale(self):
        ...
        with patch("johnston_core.infrastructure.mcp.process_client.time.monotonic", return_value=100.0):
