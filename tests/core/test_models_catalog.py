...
class TestModelsCatalog(unittest.TestCase):
    def setUp(self):
        ...
        cache_patch = patch("johnston.core.domain.policies.models_catalog.CACHE_FILE", cache_file)
        cache_patch.start()
    ...
    def test_get_context_limit_reloads_when_empty(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            catalog._limits = {}
            catalog._names = {}
            with patch("johnston.core.domain.policies.models_catalog.CONFIG_DIR", tmpdir):
    ...
    def test_get_context_limit_provider_specific_cache(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch("johnston.core.infrastructure.platform.paths.CACHE_DIR", cache_dir):
                catalog = ModelsCatalog()
    ...
    def test_save_and_load_cache(self):
        ...
        try:
            with patch("johnston.core.domain.policies.models_catalog.CACHE_FILE", tmp_path):
                cat.save_cache()
                cat2 = ModelsCatalog()
    ...
    def test_save_cache_replace_failure(self):
        ...
        try:
            cat = ModelsCatalog()
            with patch("johnston.core.domain.policies.models_catalog.CACHE_FILE", tmp_path):
                with patch("johnston.core.domain.policies.models_catalog.os.replace", side_effect=OSError("replace failed")):
                    cat.save_cache()
    ...
    def test_save_cache_cleanup_failure(self):
        ...
        try:
            ...
            with patch("johnston.core.domain.policies.models_catalog.CACHE_FILE", tmp_path):
                with patch("johnston.core.domain.policies.models_catalog.os.replace", side_effect=OSError("replace failed")):
                    with patch("johnston.core.domain.policies.models_catalog.os.remove", side_effect=OSError("remove failed")):
                        cat.save_cache()
        finally:
            for path in (tmp_path, tmp_path + ".tmp"):
                if os.path.exists(path):
...
class TestModelsCatalogAsync(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_refresh_outer_exception(self):
        cat = ModelsCatalog()
        cat._limits = {}
        with patch("johnston.core.domain.policies.models_catalog.asyncio.gather", side_effect=RuntimeError("boom")):
            limits = await cat.refresh()
        self.assertEqual(limits, {})
...
def test_get_context_window_unknown_returns_default(tmp_path):
    with patch("johnston.core.domain.policies.models_catalog.CACHE_FILE", str(tmp_path / "cache" / "c.json")):
...
def test_models_catalog_set_match_lru_bounding():
    from johnston.core.domain.policies.models_catalog import _MATCH_CACHE_MAX, _set_match
