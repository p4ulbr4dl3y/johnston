...
class TestCompactionHistory(unittest.IsolatedAsyncioTestCase):
    ...
    def test_format_compaction_title(self):
        from johnston_core.base_provider.compaction import format_compaction_title
    ...
    async def test_compact_history_resets_tool_circuit_breakers(self):
        with unittest.mock.patch("johnston_core.domain.ports.tool_registry.reset_tool_circuit_breakers") as mock_reset:
            agent = BaseAgent(api_key="mock", model="mock", base_url="https://example.com", system_prompt="", tools=[])
            self.addAsyncCleanup(agent.close)
            ...
            with unittest.mock.patch.object(
                ...
                self.assertTrue(success)
                mock_reset.assert_called_once_with("test-sess-compact")
    ...
    async def test_compact_history_calls_build_prompt_context_only_once(self):
        ...
        with (
            unittest.mock.patch("johnston_core.base_provider.tools.build_prompt_context_async", new_callable=unittest.mock.AsyncMock) as mock_bpc,
...
class TestAutoCompactionSysOverhead(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_compaction_triggers_when_sys_overhead_exceeds_threshold(self):
        ...
        with unittest.mock.patch("johnston_core.base_provider.agent.estimate_tokens", side_effect=fake_estimate):
            with unittest.mock.patch(
                "johnston_core.base_provider.BaseAgent.context_limit", new_callable=unittest.mock.PropertyMock
            ) as mock_limit:
...
class TestCompactionStreamEdgeCases(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_subagent_inherits_subagents_auto_compact_token_limit(self):
        from johnston_core.application.session.stream import configure_subagent_agent
    ...
    def test_extract_plan_from_history_tool_call(self):
        from johnston_core.base_provider.compaction import extract_plan_from_history
