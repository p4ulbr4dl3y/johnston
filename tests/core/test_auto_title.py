...
class TestAutoTitleHelpers(unittest.TestCase):
    ...
    def test_parse_session_title_multi_tier(self):
        from johnston_core.application.session.auto_title import parse_session_title
    ...
    def test_extract_first_json_object(self):
        from johnston_core.application.session.auto_title import extract_first_json_object
    ...
    def test_extract_title_from_thought(self):
        from johnston_core.application.session.auto_title import extract_title_from_thought
...
class TestAutoTitleSessionAsync(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        self.settings_patcher = patch("johnston_core.application.session.auto_title.get_settings")
    ...
    async def test_llm_auto_titling_success(self):
        ...
        with patch("johnston_core.adapters.get_adapter", return_value=mock_adapter):
    ...
    async def test_llm_auto_titling_with_provider_slash_model(self):
        ...
        with patch("johnston_core.provider_manager.ProviderManager.create_agent_for_provider", return_value=provider_agent), \
             patch("johnston_core.adapters.get_adapter", return_value=mock_adapter):
    ...
    async def test_llm_auto_titling_fallback_on_error(self):
        ...
        with patch("johnston_core.adapters.get_adapter", side_effect=RuntimeError("API error")):
            res = await auto_title_session(mock_agent, sess)
...
class TestMessageFlowAutoTitle(unittest.IsolatedAsyncioTestCase):
    async def test_schedule_auto_title_invokes_and_saves(self):
        ...
        with patch("johnston_core.application.session.auto_title.get_settings") as mock_settings:
    ...
    async def test_schedule_auto_title_skipped_when_exiting(self):
        ...
        with patch("johnston_core.application.session.auto_title.auto_title_session") as mock_title:
            app._schedule_auto_title(sess)
            await asyncio.sleep(0.05)
