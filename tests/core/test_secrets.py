...
def test_load_and_save_secrets(tmp_path):
    ...
    with patch("johnston.core.infrastructure.secrets.SECRETS_FILE", secrets_file), patch(
        "johnston.core.infrastructure.secrets.CONFIG_DIR", str(tmp_path)
    ):
        assert load_secrets() == {}
...
def test_get_secret_fallbacks(tmp_path):
    secrets_file = str(tmp_path / "secrets.json")
    with (
        patch("johnston.core.infrastructure.secrets.SECRETS_FILE", secrets_file),
        patch("johnston.core.infrastructure.secrets.CONFIG_DIR", str(tmp_path)),
