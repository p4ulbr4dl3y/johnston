...
class TestSubagentServiceOperations(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_spawn_registers_subagent_task_in_task_manager(self):
        ...
        with (
            patch("johnston_core.application.session.subagent_service.get_session_store", return_value=store),
            patch("johnston_core.application.session.subagent_service.record_subagent_session"),
            patch("johnston_core.application.session.stream.configure_subagent_agent"),
            patch("johnston_core.application.session.stream.run_subagent_stream_bg", new_callable=AsyncMock),
        ):
