...
class TestProviderAdvancedFeatures(unittest.IsolatedAsyncioTestCase):
  ...
  "test_custom": {
    ...
    "chunk_timeout": 20.0
            ...
            with patch("johnston_core.provider_manager.PROVIDERS_JSON_FILE", json_file):
                with patch("johnston_core.provider_manager.CONFIG_DIR", tmpdir):
                    pm = ProviderManager()
    ...
    def test_provider_disabling(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch("johnston_core.provider_manager.PROVIDERS_JSON_FILE", prov_file):
                with patch("johnston_core.provider_manager.CONFIG_DIR", tmpdir):
                    pm = ProviderManager()
  ...
  "test_no_max": {
    ...
    def _make_pm(self):
        ...
        self._tmpdir = tmpdir
        self._patch1 = patch("johnston_core.provider_manager.CONFIG_FILE", config_file)
        self._patch2 = patch("johnston_core.provider_manager.CONFIG_DIR", tmpdir.name)
        self._patch3 = patch("johnston_core.provider_manager.PROVIDERS_JSON_FILE", providers_file)
        self._patch1.start()
        self._patch2.start()
        self._patch3.start()
    ...
    def test_set_provider_credentials_empty_key_does_not_activate_key_required(self):
        from johnston_core.application.provider.actions import set_provider_credentials
    ...
    def test_set_provider_credentials_nonempty_key_enables_and_activates(self):
        ...
        try:
            ...
            app = MagicMock()
            with patch("johnston_core.application.provider.actions._refresh_models_background"):
