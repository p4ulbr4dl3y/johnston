...
class TestPlatformUtils(unittest.TestCase):
    def test_is_windows_real_posix(self):
        with patch("johnston_core.infrastructure.platform.platform_utils.os.name", "posix"):
            self.assertFalse(is_windows())

    def test_is_windows_real_nt(self):
        with patch("johnston_core.infrastructure.platform.platform_utils.os.name", "nt"):
    ...
    def test_shell_executable_windows_finds_second_candidate(self):
        ...
        with (
            patch("johnston_core.infrastructure.platform.platform_utils.is_windows", return_value=True),
            patch("johnston_core.infrastructure.platform.platform_utils.shutil.which", side_effect=fake_which),
        ):
    ...
    def test_shell_executable_windows_none_found(self):
        with (
            patch("johnston_core.infrastructure.platform.platform_utils.is_windows", return_value=True),
            patch("johnston_core.infrastructure.platform.platform_utils.shutil.which", return_value=None),
        ):
    ...
    def test_shell_executable_unix_falls_back_to_which(self):
        with (
            ...
            patch("johnston_core.infrastructure.platform.platform_utils.shutil.which", return_value="/usr/bin/sh"),
        ):
    ...
    def test_shell_subprocess_kwargs_windows_creationflags(self):
        with (
            patch("johnston_core.infrastructure.platform.platform_utils.is_windows", return_value=True),
            patch("johnston_core.infrastructure.platform.platform_utils.subprocess.CREATE_NEW_PROCESS_GROUP", 0x100, create=True),
        ):
    ...
    def test_shell_subprocess_kwargs_windows_no_flag(self):
        with (
            patch("johnston_core.infrastructure.platform.platform_utils.is_windows", return_value=True),
            patch("johnston_core.infrastructure.platform.platform_utils.subprocess.CREATE_NEW_PROCESS_GROUP", 0, create=True),
        ):
            self.assertEqual(shell_subprocess_kwargs(), {})

    def test_shell_subprocess_kwargs_unix(self):
        with patch("johnston_core.infrastructure.platform.platform_utils.is_windows", return_value=False):
...
class TestClipboardRetrieval(unittest.TestCase):
    ...
    def test_get_clipboard_macos_jxa_file(self):
        ...
        with (
            patch("johnston_core.infrastructure.platform.paths.TEMP_IMAGES_DIR", tmp_dir),
            patch("johnston_core.infrastructure.platform.platform_utils.is_windows", return_value=False),
            patch("johnston_core.infrastructure.platform.platform_utils.shutil.which", return_value="/usr/bin/osascript"),
            patch("johnston_core.infrastructure.platform.platform_utils.subprocess.run", return_value=res),
        ):
            file_path, img = get_clipboard_image_or_file()
    ...
    def test_get_clipboard_macos_jxa_raises(self):
        ...
        with (
            ...
            patch("johnston_core.infrastructure.platform.platform_utils.is_windows", return_value=False),
            patch("johnston_core.infrastructure.platform.platform_utils.shutil.which", return_value="/usr/bin/osascript"),
            patch("johnston_core.infrastructure.platform.platform_utils.subprocess.run", side_effect=OSError("osascript failed")),
        ):
            file_path, img = get_clipboard_image_or_file()
            self.assertIsNone(file_path)
            self.assertIsNone(img)
...
class TestTerminateProcess(unittest.TestCase):
    ...
    def test_terminate_process_unix_killpg(self):
        ...
        with (
            patch("johnston_core.infrastructure.platform.platform_utils.is_windows", return_value=False),
            patch("johnston_core.infrastructure.platform.platform_utils.os.killpg", create=True) as killpg,
        ):
    ...
    def test_terminate_process_unix_killpg_fails(self):
        ...
        with (
            patch("johnston_core.infrastructure.platform.platform_utils.is_windows", return_value=False),
            patch("johnston_core.infrastructure.platform.platform_utils.os.killpg", create=True, side_effect=OSError("no such process")),
        ):
            asyncio.run(terminate_process(process, timeout=0.5))
        process.terminate.assert_called_once()
    ...
    def test_terminate_process_windows(self):
        process = MagicMock()
        process.wait = AsyncMock()
        with patch("johnston_core.infrastructure.platform.platform_utils.is_windows", return_value=True):
            asyncio.run(terminate_process(process, timeout=0.5))
        process.terminate.assert_called_once()
    ...
    def test_terminate_process_wait_fails_then_kill(self):
        ...
        with (
            patch("johnston_core.infrastructure.platform.platform_utils.is_windows", return_value=False),
            patch("johnston_core.infrastructure.platform.platform_utils.os.killpg", create=True),
        ):
            asyncio.run(terminate_process(process, timeout=0.5))
