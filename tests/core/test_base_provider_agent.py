        ...
        with patch.object(catalog, "get_model_pricing", return_value=pricing):
            with patch.object(
                ...
                async for _ in agent_openai.stream_steps("hi"):
                    pass
            with patch("johnston_core.adapters.get_adapter", return_value=_FakeAnthropicAdapter()):
...
class TestAgentStreamEdgeCases(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_mcp_manager_ensure_tools_called_outside_pytest(self):
        ...
        with unittest.mock.patch.dict(os.environ, {"PYTEST_CURRENT_TEST": ""}):
            with unittest.mock.patch("johnston_core.infrastructure.mcp.get_mcp_manager", return_value=fake_mgr):
                with unittest.mock.patch.object(
                    agent.client.chat.completions, "create", new_callable=unittest.mock.AsyncMock
                ) as mock_create:
    ...
    async def test_attachments_processed_into_image_url(self):
        ...
        with unittest.mock.patch(
            "johnston_core.base_provider.agent.logger.warning", side_effect=lambda *a, **k: logged.append(a)
        ):
            with unittest.mock.patch.object(
                agent.client.chat.completions, "create", new_callable=unittest.mock.AsyncMock
            ) as mock_create:
    ...
    async def test_adapter_tool_call_usage_and_text_stream(self):
        ...
        with unittest.mock.patch("johnston_core.adapters.get_adapter", return_value=_FakeAdapter(streams)):
...
class TestDrainForeignSession(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_stream_steps_escalates_on_token_limit_during_reasoning(self):
        ...
        with unittest.mock.patch("johnston_core.adapters.get_adapter") as mock_get_adapter:
    ...
    def test_extract_streaming_target(self):
        from johnston_core.base_provider.agent import _extract_streaming_target
    ...
    def test_tool_schemas_parameter_orders(self):
        ...
        from johnston_core.tools.update_plan import UpdatePlanTool
    ...
    def test_extract_streaming_target_documented_boundary_tradeoff(self):
        ...
        from johnston_core.base_provider.agent import _TARGET_SCAN_BACKOFF, _extract_streaming_target
    ...
    def test_resolve_auto_compact_limit(self):
        from johnston_core.base_provider.compaction import resolve_auto_compact_limit
