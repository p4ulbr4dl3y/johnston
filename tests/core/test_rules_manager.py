...
Always run uv instead of pip.""")
    ...
    def test_deduplicate_when_global_and_project_paths_match(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            rm = RulesManager()
            with patch("johnston.core.infrastructure.runtime.markdown_scanner.CONFIG_DIR", tmpdir):
    ...
    def test_markdown_scanner_cache_ttl_skips_signature(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch("johnston.core.infrastructure.runtime.markdown_scanner.compute_dir_signature", side_effect=AssertionError("Should not compute signature within TTL")):
