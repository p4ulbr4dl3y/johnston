...
class TestRunGit(unittest.TestCase):
    def test_successful_run(self):
        with patch("johnston_core.infrastructure.runtime.git_utils.subprocess.run") as mock_run:
    ...
    def test_other_exception_returns_1(self):
        with patch("johnston_core.infrastructure.runtime.git_utils.subprocess.run", side_effect=OSError("git missing")):
...
class TestMakeGitDiff(unittest.TestCase):
    ...
    def test_fallback_when_git_unavailable(self):
        with patch("johnston_core.infrastructure.runtime.git_utils.run_git", side_effect=OSError("no git")):
            d = make_git_diff("a\nb\n", "a\nc\n", fromfile="old", tofile="new")
        self.assertIn("--- old\n+++ new", d)
        self.assertIn("-b", d)
        self.assertIn("+c", d)
    ...
    def test_fallback_when_git_errors(self):
        with patch(
            "johnston_core.infrastructure.runtime.git_utils.run_git",
    ...
    def test_identical_does_not_call_git(self):
        with patch("johnston_core.infrastructure.runtime.git_utils.run_git") as m:
            make_git_diff("a\n", "a\n")
        m.assert_not_called()
...
class TestGitContextUtils(unittest.TestCase):
    def test_is_git_repository_true(self):
        from johnston_core.infrastructure.runtime.git_utils import is_git_repository

        with patch("johnston_core.infrastructure.runtime.git_utils.run_git") as mock_git:
    ...
    def test_format_git_branch_info_named_branch(self):
        from johnston_core.infrastructure.runtime.git_utils import format_git_branch_info
    ...
    def test_format_git_branch_info_detached_head(self):
        ...
        with patch("johnston_core.infrastructure.runtime.git_utils.run_git", side_effect=side_effect):
