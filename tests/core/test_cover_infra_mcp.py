...
from johnston_core.infrastructure.mcp.manager import DEFAULT_MCP_CALL_TIMEOUT, MCPManager
...
def test_atexit_stop_all_swallows_errors(monkeypatch):
    import johnston_core.infrastructure.mcp.manager as mod
...
def test_ensure_global_config_swallows_errors(monkeypatch):
    import johnston_core.infrastructure.config.config_helpers as ch
...
async def test_update_server_config_write_error_is_swallowed(tmp_path):
    ...
    with patch("johnston_core.infrastructure.platform.platform_utils.atomic_write_json", side_effect=OSError("disk")):
...
def test_get_active_tools_creates_and_starts_clients(tmp_path):
    ...
    with patch("johnston_core.infrastructure.mcp.manager.MCPProcessClient", side_effect=[c1, c2]):
...
def test_system_prompt_snippet_skips_nameless_tool():
    ...
    snippet = m.get_system_prompt_snippet()
    assert "<mcp_servers>" in snippet
    assert "- s: f1" in snippet
