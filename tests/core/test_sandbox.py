...
def test_is_sandbox_supported():
    with (
        ...
        patch("johnston_core.infrastructure.platform.sandbox._check_seatbelt", return_value=True),
    ):
    ...
    with (
        ...
        patch("johnston_core.infrastructure.platform.sandbox._check_bwrap", return_value=True),
    ):
        assert is_sandbox_supported() is True
...
def test_build_sandboxed_command_linux_bwrap_unusable_falls_back():
    ...
    with (
        patch("platform.system", return_value="Linux"),
        patch("shutil.which", return_value="/usr/bin/bwrap"),
        patch("johnston_core.infrastructure.platform.sandbox._check_bwrap", return_value=False),
    ):
        exe, args, sandboxed = build_sandboxed_command("echo 1", cwd="/tmp/test_dir")
...
def test_check_bwrap_probes_once_and_caches(tmp_path):
    import johnston_core.infrastructure.platform.sandbox as sbx
...
def test_load_and_save_sandbox_config(tmp_path):
    from johnston_core.infrastructure.config.config_helpers import load_sandbox_config, save_sandbox_config
