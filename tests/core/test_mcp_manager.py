...
from johnston_core.infrastructure.mcp import MCPManager, MCPProcessClient
from johnston_core.infrastructure.mcp.manager import DEFAULT_MCP_CALL_TIMEOUT
from johnston_tui.app.dispatch import COMMAND_REGISTRY
...
class TestMCPManagerRegression(unittest.TestCase):
    ...
    def test_url_server_loaded_and_invalid_url_warns(self):
        ...
        with self.assertLogs("johnston_core.infrastructure.mcp.manager", level="WARNING") as captured:
            servers = mm.load_servers()
...
class TestMCPProcessClientAndExtra(unittest.TestCase):
    ...
    def test_list_changed_notification(self):
        ...
        from johnston_core.infrastructure.mcp import MCPProcessClient
...
class BugTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_failed_start_does_not_leak_cached_client(self):
        ...
        with patch("johnston_core.infrastructure.mcp.manager.MCPProcessClient", return_value=failed) as mk:
    ...
    async def test_start_timeout_does_not_leak_cached_client(self):
        ...
        with patch("johnston_core.infrastructure.mcp.manager.MCPProcessClient", return_value=hanging):
            tools = await m.get_active_tools_async()
    ...
    async def test_warm_server_async_bypasses_freshness_window(self):
        ...
        with patch("johnston_core.infrastructure.mcp.manager.MCPProcessClient", return_value=client) as mk:
    ...
    async def test_warm_server_async_ignores_disabled_or_unknown(self):
        ...
        with patch("johnston_core.infrastructure.mcp.manager.MCPProcessClient") as mk:
    ...
    async def test_failed_start_error_survives_for_status_and_clears_on_success(self):
        ...
        with patch("johnston_core.infrastructure.mcp.manager.MCPProcessClient", return_value=failed):
        ...
        with patch("johnston_core.infrastructure.mcp.manager.MCPProcessClient", return_value=ok_client):
    ...
    def test_sync_get_active_tools_records_and_clears_start_error(self):
        ...
        ok.last_error = None
        ok.tools = [{"name": "t1"}]
        with patch("johnston_core.infrastructure.mcp.manager.MCPProcessClient", return_value=ok):
            tools = m.get_active_tools()
...
class UiInteractionRegression(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_concurrent_same_server_start_shares_one_client(self):
        ...
        with patch("johnston_core.infrastructure.mcp.manager.MCPProcessClient", return_value=ok_client) as mk:
    ...
    async def test_stop_all_cancels_inflight_warmup_and_stops_half_started_client(self):
        ...
        with patch("johnston_core.infrastructure.mcp.manager.MCPProcessClient", return_value=client):
