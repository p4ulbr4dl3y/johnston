...
class TestReadResponse(unittest.TestCase):
    ...
    def test_read_response_returns_pending_response_inside_loop(self):
        ...
        with patch.object(client._response_event, "wait", side_effect=fake_wait):
            with patch("johnston_core.infrastructure.mcp.process_client.time.time", side_effect=[100.0, 100.0]):
    ...
    def test_read_response_read_task_timeout(self):
        ...
        read_task.done.return_value = False
        client._read_task = read_task
        with patch("johnston_core.infrastructure.mcp.process_client.time.time", side_effect=[100.0, 101.0]):
    ...
    def test_read_response_invalid_json_buffer_times_out(self):
        ...
        with patch("johnston_core.infrastructure.mcp.process_client.time.time", side_effect=[100.0, 100.6]):
    ...
    def test_read_response_select_empty_continues(self):
        ...
        with (
            patch("johnston_core.infrastructure.mcp.process_client.sys.platform", "linux"),
            patch("johnston_core.infrastructure.mcp.process_client.time.time", side_effect=[100.0, 100.0, 100.1]),
