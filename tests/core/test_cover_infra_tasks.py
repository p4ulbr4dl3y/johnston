...
from johnston.core.infrastructure.tasks.output import OutputBuffer, process_carriage_returns
...
def test_shell_task_repr_and_open_log_failure(monkeypatch):
    ...
    monkeypatch.setattr("johnston.core.infrastructure.tasks.shell_task.OutputLog.create", lambda _tid: Closed())
...
async def test_kill_sync_killpg_raises(monkeypatch):
    ...
    monkeypatch.setattr("johnston.core.infrastructure.tasks.shell_task.os.killpg", _boom)
    task.kill_sync()
    assert task._status.value == "killed"
