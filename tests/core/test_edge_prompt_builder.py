...
from johnston_core.application.generation.prompt_builder import (
    PromptBuilder,
    get_git_info,
    get_git_info_async,
...
def _no_git_subprocess(monkeypatch):
    ...
    monkeypatch.setattr("johnston_core.application.generation.prompt_builder._compute_git_info", lambda cwd=None: "")
    monkeypatch.setattr(
        "johnston_core.infrastructure.runtime.git_utils.format_git_branch_info", lambda cwd=None: ""
    )
...
def test_build_tools_partitioned_mcp_after_builtins(monkeypatch):
    ...
    monkeypatch.setattr("johnston_core.infrastructure.mcp.get_mcp_manager", lambda: FakeMcpManager())
...
def test_git_info_cached_short_circuits(monkeypatch):
    ...
    monkeypatch.setattr("johnston_core.application.generation.prompt_builder._compute_git_info", fake_compute)
    monkeypatch.setattr("johnston_core.application.generation.prompt_builder._GIT_INFO_CACHE", {})
    a = get_git_info()
    b = get_git_info()
