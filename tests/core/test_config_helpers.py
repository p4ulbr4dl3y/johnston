...
from johnston_core.infrastructure.config.config_helpers import (
    ensure_json_config,
    load_sandbox_config,
    load_theme_config,
    save_sandbox_config,
...
def test_catalog_cache_ttl_wired_in_refresh(monkeypatch):
    ...
    from johnston_core.models_catalog import ModelsCatalog
    ...
    monkeypatch.setattr("johnston_core.infrastructure.config.settings.get_settings", lambda: mock_settings)
    import asyncio
...
def test_load_settings_invalid_json_warns_and_defaults(caplog):
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with caplog.at_level(logging.WARNING, logger="johnston_core.infrastructure.config.settings"):
            settings = load_settings(path)
        assert settings.model is None
...
def test_session_store_disk_cache_ttl_respects_settings(monkeypatch):
    ...
    monkeypatch.setattr("johnston_core.infrastructure.storage.session_store.get_settings", lambda: settings)
...
def test_auto_compact_token_limit_settings(monkeypatch):
    from johnston_core.infrastructure.config.settings import LLMSettings, SubagentsSettings
