...
import johnston_core.infrastructure.runtime.circuit_breaker as cb_mod
...
from johnston_core.base_provider import agent as agent_mod
...
class TestAgentCompactionCoverage(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_auto_compact_divider_with_parens_and_tool_step(self):
        ...
        with patch("johnston_core.base_provider.agent.estimate_tokens", side_effect=_compaction_estimator):
            with patch("johnston_core.base_provider.BaseAgent.context_limit", new_callable=PropertyMock) as lim:
...
class TestAgentAdapterStreams(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_adapter_thought_then_text_closes_thinking(self):
        ...
        with patch("johnston_core.adapters.get_adapter", return_value=fake):
            events = []
            async for evt in agent.stream_steps("hi"):
                events.append(evt)
...
class _CompactionTokensTests(unittest.TestCase):
    def test_parse_tokens_skip_m_and_single(self):
        from johnston_core.application.session.actions import _parse_compaction_tokens
...
class TestGetRewindGitStats(unittest.IsolatedAsyncioTestCase):
    async def test_invalid_checkpoint_target_raises(self):
        from johnston_core.infrastructure.storage import git_checkpoint as gcm
...
class TestRewindExtraPaths(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_git_restore_failure_logs_warning(self):
        ...
        with patch.object(gcm.GitCheckpointManager, "restore_checkpoint", side_effect=RuntimeError("boom")):
            with patch.object(gcm.GitCheckpointManager, "purge_checkpoints_after", new=MagicMock(return_value=None)):
                with patch.object(logging.getLogger("johnston_core.application.session.actions"), "warning", side_effect=lambda *a, **k: logged.append(a)):
    ...
    async def test_rewind_chains_after_previous_restore_task(self):
        from johnston_core.application.session import actions as actions_mod
        from johnston_core.infrastructure.storage import git_checkpoint as gcm
...
class TestRecordSessionCompaction(unittest.TestCase):
    def test_record_compaction_on_agent_session(self):
        ...
        from johnston_core.infrastructure.runtime.session_interruption import record_session_compaction

        sess = AgentSession("test_id")
        record_session_compaction(sess, "Session Compacted (10k → 2k)")
        self.assertEqual(len(sess.messages), 1)
        self.assertEqual(sess.messages[0]["type"], "event_divider")
    ...
    def test_record_session_compaction_none_or_plain_object(self):
        ...
        mock_obj = MagicMock()
        record_session_compaction(mock_obj, "Compacted")
        mock_obj.add_event.assert_called_once_with({"type": "event_divider", "text": "Compacted"})
