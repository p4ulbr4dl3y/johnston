...
from johnston_tui.app.command_provider import (
    _build_command_suggestions,
    get_all_command_suggestions,
)
from johnston_tui.app.dispatch import (
    _load_skill_blocks,
    _resolve_skills,
    build_command_registry,
    handle_slash_command,
)
from johnston_tui.app.role_service import reconcile_active_agent, toggle_agent_role
from johnston_tui.app.session_state import collect_session_data, recompute_context_tokens
...
class TestSessionStateCoverage:
    ...
    def test_collect_session_data_no_user_messages(self):
        ...
        with patch("johnston_tui.app.session_state.is_ui_visible_user_message", return_value=False):
            assert collect_session_data(app) is None
    ...
    def test_collect_session_data_success_with_agent(self):
        ...
        with patch("johnston_tui.app.session_state.is_ui_visible_user_message", return_value=True):
    ...
    def test_recompute_context_tokens_calculation(self):
        ...
        with patch("johnston_core.application.generation.prompt_builder.PromptBuilder") as mock_builder_cls, patch(
            "johnston_core.infrastructure.runtime.token_util.estimate_tokens", side_effect=lambda x: len(str(x))
        ):
...
class TestRoleServiceCoverage:
    def test_toggle_agent_role_standard(self):
        ...
        with patch("johnston_core.role_registry.RoleRegistry.get_instance", return_value=mock_registry):
...
class TestDispatchCoverage:
    ...
    def test_load_skill_blocks_read_file_error_fallback(self, tmp_path):
        ...
        with patch("johnston_core.infrastructure.runtime.frontmatter.parse_frontmatter", side_effect=Exception("parse error")):
    ...
    async def test_handle_slash_command_registered_command(self):
        ...
        with patch.dict("johnston_tui.app.dispatch.COMMAND_REGISTRY", {"/mock": mock_cmd_cls}, clear=False):
    ...
    async def test_handle_slash_command_registered_with_cyrillic_homoglyphs(self):
        ...
        with patch.dict("johnston_tui.app.dispatch.COMMAND_REGISTRY", {"/clear": mock_cmd_cls}, clear=False):
    ...
    async def test_handle_slash_command_skills_with_user_request(self):
        ...
        with patch("johnston_tui.app.dispatch.get_skill_manager") as mock_sm_get:
            sm = MagicMock()
    ...
    async def test_handle_slash_command_mcp_prompt_fallback(self):
        ...
        with patch("johnston_tui.app.dispatch.get_skill_manager") as mock_sm_get, patch(
            "johnston_core.infrastructure.mcp.get_mcp_manager", return_value=mock_mm
        ):
    ...
    async def test_handle_slash_command_mcp_exception(self):
        ...
        with patch("johnston_tui.app.dispatch.get_skill_manager") as mock_sm_get, patch(
            "johnston_core.infrastructure.mcp.get_mcp_manager", side_effect=RuntimeError("MCP failed")
        ):
            mock_sm_get.return_value.get_skill.return_value = None
...
class TestCommandProviderCoverage:
    def test_build_command_suggestions_all_sources(self):
        ...
        with patch.dict("johnston_tui.app.command_provider.COMMAND_REGISTRY", mock_registry, clear=True), patch(
            "johnston_tui.app.command_provider.get_skill_manager", return_value=mock_sm
        ), patch("johnston_core.infrastructure.mcp.get_mcp_manager", return_value=mock_mm):
    ...
    def test_build_command_suggestions_with_exceptions(self):
        ...
        with patch.dict("johnston_tui.app.command_provider.COMMAND_REGISTRY", mock_registry, clear=True), patch(
            "johnston_tui.app.command_provider.get_skill_manager", side_effect=Exception("skill manager error")
        ), patch("johnston_core.infrastructure.mcp.get_mcp_manager", side_effect=Exception("mcp error")):
            suggestions = _build_command_suggestions()
    ...
    async def test_get_all_command_suggestions_caching(self):
        import johnston_tui.app.command_provider as cp
        ...
        try:
            with patch("johnston_tui.app.command_provider._build_command_suggestions", return_value=sample_suggestions) as mock_b:
                ...
                with patch("johnston_tui.app.command_provider.time.time", return_value=cp._command_suggestions_cache_time + 15.0):
