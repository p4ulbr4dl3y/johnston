...
class TestInvokeSubagentTool(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_invoke_subagent_branch_equals_current_creates_isolated_branch(self):
        ...
        with (
            patch("johnston.core.infrastructure.runtime.subagent_worktree.SubagentWorktreeManager.is_git_repo", return_value=True),
            patch("johnston.core.application.session.subagent_service.run_git_async", new_callable=AsyncMock) as mock_git,
            patch("johnston.core.infrastructure.runtime.subagent_worktree.SubagentWorktreeManager.create_worktree_async", new_callable=AsyncMock) as mock_wt,
        ):
            mock_git.return_value.stdout = "main\n"
            mock_wt.side_effect = lambda pdir, sid, branch: (f"/tmp/wt/{sid}", branch)
    ...
    async def test_kill_subagent_cleans_up_worktree_and_branch(self):
        ...
        with patch("johnston.core.application.session.subagent_service.SubagentWorktreeManager.cleanup_worktree") as mock_cleanup:
    ...
    def test_prune_merged_subagent_branches(self):
        ...
        with (
            ...
            patch("johnston.core.infrastructure.runtime.subagent_worktree.run_git") as mock_run_git,
        ):
            mock_res = MagicMock()
