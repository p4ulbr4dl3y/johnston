...
from johnston_core.domain.policies.permission_policy import PermissionAction, PermissionDecision
from johnston_core.infrastructure.runtime.tool_name import normalize_tool_name
from johnston_core.tools.registry import REGISTRY, execute_tool, get_default_tools
...
class TestRegistry(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_execute_tool_edit_routes_to_edit_tool(self):
        ...
        from johnston_core.tools.registry import REGISTRY

        self.assertIs(REGISTRY["edit"], EditTool)
        mock_pm = MagicMock()
        mock_pm.check_permission.return_value = PermissionDecision(PermissionAction.ALLOW, "")
        with (
            patch("johnston_core.permission_manager.PermissionManager.get_instance", return_value=mock_pm),
    ...
    async def test_execute_tool_execution_exception(self):
        ...
        with patch.object(REGISTRY["read"], "execute", side_effect=RuntimeError("Execute failed")):
            with self.assertLogs("johnston_core.tools.registry", level="WARNING") as cm:
    ...
    async def test_execute_tool_mcp_disallowed_in_mode(self):
        ...
        with (
            patch("johnston_core.infrastructure.mcp.get_mcp_manager", return_value=mock_mcp_mgr),
            patch("johnston_core.role_registry.RoleRegistry.get_instance", return_value=mock_role_registry),
        ):
            mock_app = MagicMock()
            mock_app.role = "plan"
    ...
    async def test_execute_tool_mcp_error_exception(self):
        ...
        with (
            patch("johnston_core.infrastructure.mcp.get_mcp_manager", return_value=mock_mcp_mgr),
            patch("johnston_core.role_registry.RoleRegistry.get_instance", return_value=mock_role_registry),
            self.assertLogs("johnston_core.tools.registry", level="WARNING") as cm,
        ):
    ...
    def _mock_mode(self, name="action"):
        ...
        mock_role_registry = MagicMock()
        mock_role_registry.get_role.return_value = mock_mode_def
        return patch("johnston_core.role_registry.RoleRegistry.get_instance", return_value=mock_role_registry)
    ...
    async def test_execute_tool_permission_denied(self):
        ...
        with patch("johnston_core.permission_manager.PermissionManager.get_instance", return_value=mock_pm):
            res = await execute_tool("read", {"path": "foo.txt"})
    ...
    async def test_execute_tool_permission_ask_terminal_interactive_allow(self):
        ...
        with (
            ...
            patch("johnston_core.tools.read.ReadTool.execute", return_value="file content"),
        ):
            res = await execute_tool("read", {"path": "foo.txt"})
    ...
    async def test_execute_tool_mcp_async_call(self):
        ...
        with patch("johnston_core.infrastructure.mcp.get_mcp_manager", return_value=_FakeMCPManager()), self._mock_mode():
    ...
    async def test_mcp_tool_confirms_with_server_name(self):
        ...
        with (
            patch("johnston_core.permission_manager.PermissionManager.get_instance", return_value=mock_pm),
            patch("johnston_core.infrastructure.mcp.get_mcp_manager", return_value=mock_mcp),
        ):
