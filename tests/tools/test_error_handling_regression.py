...
from johnston.core.tools import ask_user, edit
...
class TestMCPListToolsFailure(unittest.IsolatedAsyncioTestCase):
    def _mode(self):
        ...
        return patch("johnston.core.application.roles.role_registry.RoleRegistry.get_instance", return_value=reg)
    ...
    async def test_get_active_tools_exception_wrapped(self):
        ...
        with (
            patch("johnston.core.infrastructure.mcp.get_mcp_manager", return_value=mock_mgr),
            self._mode(),
        ):
...
class TestMCPNameMissCache(unittest.IsolatedAsyncioTestCase):
    ...
    def setUp(self):
        from johnston.core.tools import registry
