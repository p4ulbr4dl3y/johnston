...
from johnston_core.tools.web_fetch import WebFetchTool
...
class TestWebFetchTool(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_fetch_oversize_content_length_rejected(self, mock_client_cls):
        ...
        from johnston_core.tools.utils import MAX_TOOL_PAYLOAD_BYTES
    ...
    async def test_convert_content_to_md_sync(self):
        from johnston_core.tools.web_fetch import _convert_content_to_md_sync

        with patch("johnston_core.infrastructure.converter.convert_bytes", return_value="converted md"):
    ...
    async def test_client_timeout_uses_configured_value(self, mock_client_cls):
        ...
        from johnston_core.infrastructure.config.settings import JohnstonSettings, ToolsSettings
    ...
    async def test_payload_cap_honors_configured_value(self, mock_client_cls):
        # Regression: the payload size cap must come from johnston_core.tools.max_tool_payload_bytes.
        with patch("johnston_core.tools.web_fetch.get_max_tool_payload_bytes", return_value=100):
            response = MagicMock()
            response.status_code = 200
    ...
    @patch("httpx.AsyncClient")
    @patch("johnston_core.tools.read.convert_doc_to_markdown_sync", side_effect=RuntimeError("no converter"))
    ...
    def test_sanitize_web_content_strips_script_and_style_blocks(self):
        from johnston_core.tools.web_fetch import _sanitize_web_content
    ...
    def test_decode_response_bytes_encoding_and_fallback(self):
        from johnston_core.tools.web_fetch import _decode_response_bytes
    ...
    @patch("johnston_core.tools.web_fetch._dns_cache_policy", return_value=(60.0, 2))
    @patch("socket.getaddrinfo")
    async def test_dns_lru_cache_eviction(self, mock_gai, mock_policy):
        from johnston_core.tools.web_fetch import _DNS_CACHE, _is_private_host
