...
from johnston_core.tools.shell import ShellTool, _new_task_id
...
def _reset_permissions():
    ...
    with patch("johnston_core.permission_manager.CONFIG_FILE", "/nonexistent_test_config.json"):
        yield
    pm.clear_session_overrides()
...
async def test_windows_execution_branch(tool):
    with (
        patch("johnston_core.tools.shell.is_windows", return_value=True),
...
async def test_subprocess_creation_exception_cleanup_no_transport(tool):
    with (
        patch("johnston_core.tools.shell.is_windows", return_value=False),
...
async def test_create_windows_process_powershell(tool):
    with (
        patch(
            "johnston_core.tools.shell.shell_executable",
...
async def test_create_windows_process_cmd(tool):
    with (
        patch("johnston_core.tools.shell.shell_executable", return_value="C:\\Windows\\System32\\cmd.exe"),
        patch("asyncio.create_subprocess_exec", new_callable=AsyncMock) as mock_exec,
    ):
...
async def test_create_windows_process_default_shell(tool):
    from johnston_core.infrastructure.platform.platform_utils import shell_subprocess_kwargs

    with (
        patch("johnston_core.tools.shell.shell_executable", return_value="/bin/sh"),
...
async def test_command_timeout_terminates_process(tool, make_app_mock, make_tool_context):
    ...
    with (
        ...
        patch("johnston_core.tools.shell.terminate_process", new_callable=AsyncMock) as mock_term,
    ):
...
async def test_move_to_background_during_sync_execution(tool, make_app_mock, make_tool_context):
    ...
    with (
        ...
        patch("johnston_core.tools.shell.shell_executable", return_value="/bin/sh"),
        patch("johnston_core.tools.shell.terminate_process", new_callable=AsyncMock),
    ):
        exec_task = asyncio.create_task(tool.execute({"command": "tail -f log.txt"}, ctx=ctx))
        await task_started.wait()

        # Foreground sync task is isolated from TaskManager while running
        assert len([t for t in app.task_manager]) == 0
        tasks = list(getattr(app, "_foreground_shell_tasks", {}).values())
        assert len(tasks) == 1
        task = tasks[0]
...
async def test_main_sync_task_isolated_from_task_manager(tool, make_app_mock, make_tool_context):
    app = _app(make_app_mock, task_manager=TaskManager())
    ctx = make_tool_context(app=app)
    ...
    with (
        ...
        # Foreground sync task is isolated from TaskManager while running
        assert len([t for t in app.task_manager]) == 0
        assert len(getattr(app, "_foreground_shell_tasks", {})) == 1
        exec_task.cancel()
        with pytest.raises(asyncio.CancelledError):
            await exec_task
    assert len([t for t in app.task_manager]) == 0
    assert len(getattr(app, "_foreground_shell_tasks", {})) == 0
...
async def test_main_sync_read_task_drain_timeout(tool, make_app_mock, make_tool_context):
    ...
    with (
        patch.object(ShellTool, "_create_std_process", return_value=p),
        patch("johnston_core.tools.shell.asyncio.wait_for", side_effect=custom_wait_for),
    ):
...
async def test_background_task_kill_lifecycle(tool, make_app_mock):
    ...
    killer = KillTool()
    with patch("johnston_core.tools.shell.shell_executable", return_value="/bin/sh"):
...
async def test_shell_readonly_sandbox_unsupported_fails_closed(tool, make_tool_context):
    ...
    with patch("johnston_core.infrastructure.platform.sandbox.is_sandbox_supported", return_value=False):
        res = await tool.execute({"command": "echo test"}, ctx=ctx)
    assert res.is_error
...
async def test_shell_readonly_windows_mutations_blocked():
    from johnston_core.tools.shell import _check_read_only_command_mutations
...
def inspect_has_no_destructive_guard():
    ...
    from johnston_core.tools import shell as shell_mod
...
async def test_shell_sandbox_enabled_invokes_sandboxed_command(tool, make_tool_context):
    ...
    with patch("johnston_core.infrastructure.platform.sandbox.build_sandboxed_command") as mock_build, \
...
async def test_shell_sandbox_unavailable_surfaces_banner(tool, make_tool_context):
    ...
    ctx.host.sandbox_enabled = True
    with patch("johnston_core.infrastructure.platform.sandbox.build_sandboxed_command") as mock_build, \
         patch("johnston_core.infrastructure.platform.sandbox.is_sandbox_supported", return_value=False), \
         patch("asyncio.create_subprocess_exec") as mock_exec:
...
async def test_shell_sandbox_no_banner_when_supported(tool, make_tool_context):
    ...
    ctx.host.sandbox_enabled = True
    with patch("johnston_core.infrastructure.platform.sandbox.build_sandboxed_command") as mock_build, \
         patch("johnston_core.infrastructure.platform.sandbox.is_sandbox_supported", return_value=True), \
         patch("asyncio.create_subprocess_exec") as mock_exec:
...
async def test_shell_auto_derives_idle_timeout(tool, make_app_mock, make_tool_context):
    ...
    with (
        ...
        patch("johnston_core.tools.shell.ShellTask") as mock_task_cls,
    ):
        mock_task = MagicMock()
...
def test_apply_role_tools_cleans_timeout_description(tool):
    from johnston_core.domain.policies.role_policy import AgentRole
...
async def test_shell_cwd_sandbox_read_blocked(tool, make_app_mock, make_tool_context):
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with patch("johnston_core.infrastructure.platform.sandbox.is_path_readable_in_sandbox", return_value=False):
...
async def test_shell_cwd_sandbox_write_blocked(tool, make_app_mock, make_tool_context):
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        ctx = make_tool_context(app=app, cwd=tmpdir)
        with (
            patch("johnston_core.infrastructure.platform.sandbox.is_path_readable_in_sandbox", return_value=True),
            patch("johnston_core.infrastructure.platform.sandbox.is_path_writable_in_sandbox", return_value=False),
        ):
            res = await tool.execute({"command": "echo ok", "cwd": tmpdir}, ctx=ctx)
            assert res.is_error
...
def test_attach_shell_widget_sync_vs_background():
    from johnston_core.tools.shell import _attach_shell_widget
...
async def test_promote_task_to_background_helper(make_app_mock, make_tool_context):
    from johnston_core.tools.shell import _promote_task_to_background
...
async def test_cancel_read_task_helper():
    from johnston_core.tools.shell import _cancel_read_task
