...
class TestTools(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_read_tool(self):
        ...
        self.assertIn("exceeds line count", res_oob.display)
        self.assertIn("File has 5 lines", res_oob.display)
        ...
        with patch("johnston.core.tools.read.tool.convert_doc_to_markdown_sync", return_value="# Converted PDF Header\nPDF body text"):
    ...
    async def test_read_tool_doc_caching(self):
        from johnston.core.tools.read import convert_doc_to_markdown_sync
        ...
        with patch("johnston.core.tools.read.doc.set_cached_doc_markdown") as mock_set:
            with patch("johnston.core.infrastructure.converter.convert_file", return_value="# Cached Doc Header\nDoc text"):
