...
from johnston_core.tools.read import (
    _DOC_CACHE,
    MAX_DOC_CACHE,
    ReadTool,
...
class TestReadToolCoverage(unittest.IsolatedAsyncioTestCase):
    ...
    def test_convert_doc_to_markdown_sync_cache_hit(self):
        ...
        with patch("johnston_core.tools.read.doc.get_cached_doc_markdown", return_value="# Cached Doc"):
    ...
    def test_convert_doc_to_markdown_sync_success(self):
        ...
        with (
            patch("johnston_core.tools.read.doc.get_cached_doc_markdown", return_value=None),
            patch("johnston_core.infrastructure.converter.convert_file", return_value="# Doc Output"),
        ):
            res = convert_doc_to_markdown_sync(fake_path)
    ...
    def test_convert_doc_to_markdown_sync_failure_raises(self):
        ...
        with (
            patch("johnston_core.tools.read.doc.get_cached_doc_markdown", return_value=None),
            patch("johnston_core.infrastructure.converter.convert_file", side_effect=RuntimeError("fail")),
        ):
    ...
    def test_empty_conversion_result_not_cached(self):
        ...
        try:
            with (
                patch("johnston_core.tools.read.doc.get_cached_doc_markdown", return_value=None),
                patch("johnston_core.infrastructure.converter.convert_file", return_value=""),
            ):
                res = convert_doc_to_markdown_sync(fake_path)
    ...
    async def test_read_doc_conversion_error(self):
        ...
        with patch("johnston_core.tools.read.tool.convert_doc_to_markdown_sync", side_effect=RuntimeError("Doc convert fail")):
    ...
    @patch("johnston_core.tools.read.tool.convert_doc_to_markdown_sync")
