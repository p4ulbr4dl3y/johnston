...
class TestShellEdge(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_empty_command_safe(self):
        res = str(await self.tool.execute({"command": ""}, ctx=self._ctx()))
    ...
    async def test_none_command_safe(self):
        res = str(await self.tool.execute({"command": None}, ctx=self._ctx()))
        self.assertEqual(res, "(no output)")

    # ---------- special chars / metacharacters -------------

    async def test_semicolon_runs_both(self):
        res = str(await self.tool.execute({"command": "echo a; echo b"}, ctx=self._ctx()))
        self.assertIn("a", res)
        self.assertIn("b", res)
    ...
    async def test_or_short_circuit(self):
        ...
        res = str(await self.tool.execute({"command": "echo ok || echo SHOULD_NOT_RUN"}, ctx=self._ctx()))
        self.assertIn("ok", res)
        self.assertNotIn("SHOULD_NOT_RUN", res)

    async def test_pipe_chains(self):
        res = str(await self.tool.execute({"command": "echo pipe_data | cat"}, ctx=self._ctx()))
        self.assertIn("pipe_data", res)

    async def test_redirect_stdout_to_devnull_loses_output(self):
        res = str(await self.tool.execute({"command": "echo hidden > /dev/null"}, ctx=self._ctx()))
        self.assertNotIn("hidden", res)

    async def test_redirect_append(self):
        res = str(await self.tool.execute({"command": "echo x >> /dev/null"}, ctx=self._ctx()))
        self.assertEqual(res, "(no output)")

    async def test_input_redirect(self):
        res = str(await self.tool.execute({"command": "echo from_stdin < /dev/null"}, ctx=self._ctx()))
        self.assertIn("from_stdin", res)

    async def test_command_substitution_runs(self):
        res = str(await self.tool.execute({"command": "echo dollar_$(echo sub)"}, ctx=self._ctx()))
        # $(...) runs -> output contains "sub"
        self.assertIn("sub", res)

    async def test_backtick_substitution_runs(self):
        res = str(await self.tool.execute({"command": "echo bt_`echo sub2`"}, ctx=self._ctx()))
        self.assertIn("sub2", res)

    async def test_arithmetic_substitution(self):
        res = str(await self.tool.execute({"command": "echo $((2+3))"}, ctx=self._ctx()))
        self.assertIn("5", res)

    async def test_not_bang_operator(self):
        # `! echo -n ; echo $?` → negation gives exit 1 → $? == 1
        res = str(await self.tool.execute(
    ...
    async def test_wildcard_glob_expands(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            with open(os.path.join(d, "banana.txt"), "w") as f:
                f.write("b")
            res = str(await self.tool.execute({"command": "echo *"}, ctx=self._ctx(cwd=d)))
        self.assertIn("apple.txt", res)
        self.assertIn("banana.txt", res)

    async def test_tilde_expands(self):
        res = str(await self.tool.execute({"command": "echo ~"}, ctx=self._ctx()))
        self.assertTrue(os.path.isabs(res.strip()))

    async def test_nested_quotes(self):
        res = str(await self.tool.execute({"command": "echo \"outer 'inner' quote\""}, ctx=self._ctx()))
    ...
    async def test_unicode_and_emoji_roundtrip(self):
        ...
        res = str(await self.tool.execute({"command": f"echo {shlex.quote(payload)}"}, ctx=self._ctx()))
        self.assertIn(payload, res)

    async def test_argv_with_spaces_quoted(self):
        res = str(await self.tool.execute({"command": 'echo "a b  c" d'}, ctx=self._ctx()))
        self.assertIn("a b  c d", res)

    async def test_very_long_command_many_args(self):
        args = " ".join(f"a{i}" for i in range(5000))
        res = str(await self.tool.execute({"command": f"echo {args}"}, ctx=self._ctx()))
    ...
    async def test_false_exit_code_is_lost(self):
        ...
        res = str(await self.tool.execute({"command": "false"}, ctx=self._ctx()))
        self.assertEqual(res, "(no output)")

    async def test_explicit_exit_code_is_lost(self):
        # Never-reached output + non-zero exit → tool reports nothing, no code.
        res = str(await self.tool.execute({"command": "echo never_shown; exit 7"}, ctx=self._ctx()))
    ...
    async def test_stderr_only_captured(self):
        res = str(await self.tool.execute({"command": "echo stderr_only 1>&2"}, ctx=self._ctx()))
        self.assertIn("stderr_only", res)

    async def test_kill_signal_behaves(self):
        # self-kill → shell reports no output; must not crash.
        res = str(await self.tool.execute({"command": "kill -9 $$"}, ctx=self._ctx()))
        self.assertIsInstance(res, str)

    # ---------- unicode/binary output, CRLF, huge output ----------

    async def test_binary_bytes_no_crash(self):
        res = str(await self.tool.execute({"command": "printf '\\x01\\x02\\xff\\x00data'"}, ctx=self._ctx()))
        self.assertIn("data", res)

    async def test_crlf_normalized(self):
        res = str(await self.tool.execute({"command": "printf 'one\\r\\ntwo\\r\\n'"}, ctx=self._ctx()))
    ...
    async def test_very_large_output_truncated(self):
        ...
        res = str(await self.tool.execute({"command": cmd}, ctx=self._ctx()))
    ...
    async def test_nonexistent_cwd_falls_back(self):
        ...
        res = str(await self.tool.execute({"command": "echo cwd_fallback"}, ctx=ctx))
        self.assertIn("cwd_fallback", res)

    async def test_relative_cwd(self):
        res = str(await self.tool.execute({"command": "pwd"}, ctx=self._ctx()))
        self.assertTrue(os.path.isabs(res.strip()))
    ...
    async def test_env_inherited(self):
        ...
        try:
            res = str(await self.tool.execute({"command": f"echo got=${{{key}}}"}, ctx=self._ctx()))
        finally:
            del os.environ[key]
        self.assertIn("got=inherited_value", res)

    async def test_path_unset_command_not_found(self):
        res = str(await self.tool.execute({"command": "PATH=/nonexistent_path_zz echo hi; echo $?"}, ctx=self._ctx()))
    ...
    async def test_missing_env_var_empty(self):
        ...
        res = str(await self.tool.execute({"command": f"echo val=[${{{key}}}]"}, ctx=self._ctx()))
    ...
    async def test_non_executable_script_denied(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            res = str(await self.tool.execute({"command": script}, ctx=self._ctx(cwd=d)))
            self.assertNotIn("SHOULD_NOT_RUN", res)
    ...
    async def test_rm_rf_not_blocked_default(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            os.mkdir(victim)
            res = str(await self.tool.execute({"command": f"rm -rf {shlex.quote(victim)}"}, ctx=self._ctx()))
