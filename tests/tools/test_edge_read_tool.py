...
async def test_invalid_utf8_with_line_window(tmp_path, ctx):
    ...
    res = await ReadTool().execute({"path": p, "start_line": 1, "end_line": 3}, ctx=ctx)
    assert "ERR:" not in res, res
...
async def test_empty_file(tmp_path, ctx):
    ...
    res = await ReadTool().execute({"path": p}, ctx=ctx)
    assert "0 lines" in res, res


@pytest.mark.asyncio
async def test_empty_file_with_line_window(tmp_path, ctx):
    p = wb(tmp_path, "empty2.txt", b"")
    res = await ReadTool().execute({"path": p, "start_line": 1, "end_line": 5}, ctx=ctx)
    assert "0 lines" in res, res
...
async def test_file_no_trailing_newline_counts_as_line(tmp_path, ctx):
    ...
    res = await ReadTool().execute({"path": p}, ctx=ctx)
    assert "1 line" in res or "Lines 1-1 of 1" in res, res
...
async def test_oversized_read_request_returns_without_crash(tmp_path, ctx):
    ...
    res = await ReadTool().execute({"path": p, "start_line": 999}, ctx=ctx)
...
async def test_start_line_exceeds_total_single_line_file(tmp_path, ctx):
    ...
    res = await ReadTool().execute({"path": p, "start_line": 5}, ctx=ctx)
    assert "ERR:" in res, res
...
async def test_symlink_resolves_to_target(tmp_path, ctx):
    ...
    res = await ReadTool().execute({"path": link}, ctx=ctx)
...
async def test_hard_link_shares_content(tmp_path, ctx):
    ...
    os.link(p, hlink)
    res = await ReadTool().execute({"path": hlink}, ctx=ctx)
...
async def test_relative_path_resolves_against_cwd(ctx, tmp_path):
    ...
    res = await ReadTool().execute({"path": "rel.txt"}, ctx=ctx)
...
async def test_dotdot_traversal_reads_file(tmp_path, ctx):
    ...
    res = await ReadTool().execute({"path": "../outside.txt"}, ctx=ctx)
...
async def test_directory_path_returns_listing(tmp_path, ctx):
    ...
    res = await ReadTool().execute({"path": os.path.join(tmp_path, "adir")}, ctx=ctx)
...
async def test_absolute_path_outside_cwd(tmp_path, ctx):
    ...
    res = await ReadTool().execute({"path": str(outside)}, ctx=ctx)
...
async def test_expanduser_tilde(tmp_path, ctx):
    ...
    try:
        with open(p, "w") as f:
            f.write("tilde read\n")
        res = await ReadTool().execute({"path": "~/.johnston_read_test_tmp.txt"}, ctx=ctx)
...
async def test_etc_passwd_readable(tmp_path, ctx):
    ...
    res = await ReadTool().execute({"path": "/etc/hosts"}, ctx=ctx)
...
async def test_missing_path(tmp_path, ctx):
    res = await ReadTool().execute({}, ctx=ctx)
...
async def test_path_none(tmp_path, ctx):
    res = await ReadTool().execute({"path": None}, ctx=ctx)
    assert "ERR: listing" not in res


@pytest.mark.asyncio
async def test_path_non_string(tmp_path, ctx):
    res = await ReadTool().execute({"path": 12345}, ctx=ctx)
...
async def test_extra_unknown_keys_ignored(tmp_path, ctx):
    ...
    res = await ReadTool().execute({"path": p, "bogus": 1, "wat": "x"}, ctx=ctx)
...
async def test_start_end_inverted(tmp_path, ctx):
    ...
    res = await ReadTool().execute({"path": p, "start_line": 4, "end_line": 1}, ctx=ctx)
    assert "ERR:" not in res, res
...
async def test_start_end_beyond_file(tmp_path, ctx):
    ...
    res = await ReadTool().execute({"path": p, "start_line": 1, "end_line": 500}, ctx=ctx)
    assert "ERR:" not in res, res
...
async def test_end_none_with_start(tmp_path, ctx):
    ...
    res = await ReadTool().execute({"path": p, "start_line": 2, "end_line": None}, ctx=ctx)
    assert "l2" in res
...
async def test_start_end_as_strings(tmp_path, ctx):
    ...
    res = await ReadTool().execute({"path": p, "start_line": "2", "end_line": "3"}, ctx=ctx)
...
async def test_start_as_float(tmp_path, ctx):
    ...
    res = await ReadTool().execute({"path": p, "start_line": 2.7}, ctx=ctx)
...
async def test_start_as_non_numeric_string(tmp_path, ctx):
    ...
    res = await ReadTool().execute({"path": p, "start_line": "abc"}, ctx=ctx)
    assert "ERR:" not in res, res
...
async def test_start_huge_integer(tmp_path, ctx):
    ...
    res = await ReadTool().execute({"path": p, "start_line": 10**30}, ctx=ctx)
    assert "ERR:" in res
...
async def test_content_offset_beyond_eof(tmp_path, ctx):
    ...
    res = await ReadTool().execute({"path": p, "content_offset": 5000}, ctx=ctx)
    assert "ERR:" not in res, res
...
async def test_content_offset_negative_clamped(tmp_path, ctx):
    ...
    res = await ReadTool().execute({"path": p, "content_offset": -100}, ctx=ctx)
...
async def test_content_offset_zero(tmp_path, ctx):
    ...
    res = await ReadTool().execute({"path": p, "content_offset": 0}, ctx=ctx)
...
async def test_crlf_with_line_window(tmp_path, ctx):
    ...
    res = await ReadTool().execute({"path": p, "start_line": 2, "end_line": 2}, ctx=ctx)
    assert "b" in res
...
async def test_line_window_only_returns_requested_lines(tmp_path, ctx):
    ...
    res = await ReadTool().execute({"path": p, "start_line": 3, "end_line": 4}, ctx=ctx)
    assert "l3" in res
    assert "l4" in res
...
async def test_end_line_gt_total_shows_all(tmp_path, ctx):
    ...
    res = await ReadTool().execute({"path": p, "end_line": 100}, ctx=ctx)
