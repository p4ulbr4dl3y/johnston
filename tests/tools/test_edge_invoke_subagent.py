...
from johnston_core.domain.entities.session import SessionStatus
from johnston_core.infrastructure.storage.session_store import SessionStore
from johnston_core.infrastructure.tasks.output import MAX_SUBAGENT_RESULT_CHARS
...
async def test_role_falls_back_or_accepts(role, monkeypatch):
    ...
    monkeypatch.setattr("johnston_core.role_registry.RoleRegistry.get_role", fake_get_role)
    monkeypatch.setattr("johnston_core.role_registry.RoleRegistry.load_roles", lambda self, *a, **k: {})
...
async def test_main_scope_role_falls_back_to_worker(monkeypatch):
    ...
    monkeypatch.setattr("johnston_core.role_registry.RoleRegistry.get_role", fake_get)
    monkeypatch.setattr("johnston_core.role_registry.RoleRegistry.load_roles", lambda self, *a, **k: {})
...
async def test_role_pinned_provider_not_connected_raises(monkeypatch):
    ...
    monkeypatch.setattr("johnston_core.provider_manager.ProviderManager", _Pm)
...
async def test_worktree_create_raises_crashes_instead_of_err(monkeypatch):
    ...
    monkeypatch.setattr("johnston_core.tools.invoke_subagent.SubagentWorktreeManager", _BadWorktree)
    try:
...
async def test_session_create_returns_none_crashes(monkeypatch):
    ...
    try:
        ...
        import johnston_core.infrastructure.storage.session_store as sm
