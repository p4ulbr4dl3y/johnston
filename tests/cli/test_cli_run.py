...
class TestCLIRun(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_run_headless_async_apply_role_called(self):
        ...
        with patch("johnston.cli.commands.run_cmd.apply_role") as mock_apply:
            out_buf = io.StringIO()
            with redirect_stdout(out_buf):
                code = await run_headless_async(args, pm=pm)
            self.assertEqual(code, 0)
    ...
    @patch("johnston.core.application.skills.manager.get_skill_manager")
    ...
    @patch("johnston.core.infrastructure.storage.session_store.SessionStore.get_instance")
...
class TestCLIRunSyncWrapper(unittest.TestCase):
    ...
    @patch("johnston.cli.commands.run_cmd.run_headless_async")
    ...
    @patch("johnston.cli.commands.run_cmd.run_headless", return_value=0)
