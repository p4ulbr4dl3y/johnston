...
Iteration N (1..3):
  [1. Run Tests via shell] ──(fail)──> [Fix Baseline]
          │ (pass)
          ▼
  [2. invoke_subagent(type="reviewer")] ──> Yield turn & wait for runtime notification
          │
          ▼
  [3. Parse Reviewer Output]
          ...
          └── VERDICT: REJECT ───────────> [Extract P0/P1/P2 Blockers (Ignore P3)]
                                     ...
                                     [4. Apply Surgical Fixes]
                                        ├── Small: edit directly
                                        └── Large: invoke_subagent(type="worker") -> git merge
                                                 │
                                                 ▼
...
invoke_subagent(
    ...
    prompt="Audit changes in <scope> via git diff/files and execute adversarial verification via shell (probe edge cases, boundary inputs, failure paths). Return findings with severity P0-P3 and final VERDICT.",
    type="reviewer",
)
```
...
- **Large fixes / multi-file refactors**: Prevent main context exhaustion by delegating to `invoke_subagent(type="worker")`. Worker executes in an isolated git worktree; apply with `git merge <subagent-branch>` upon completion.
