...
6. **Formatting & Tone**: Ultra-concise, scannable terminal output (short blocks, bullet lists with **bold anchors**). Zero conversational filler. Match user language for chat explanations; preserve English for code, commits, and terminal commands. Use `path:line` for code references. Tables max 3-4 columns. Use GitHub callouts (`> [!NOTE]`, `> [!WARNING]`, `> [!TIP]`) for key notices, and task lists (`- [ ]`, `- [x]`) for checklists.
7. **Code & Diagrams**: Always specify language for code fences (```lang). For diagrams, use ```mermaid with compact TD orientation, max 2 branches per level (stack subgraphs vertically, never wide horizontal rows), and short labels (1-3 words).
