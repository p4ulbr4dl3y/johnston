...
<guidelines>
1. Grounding: Anchor all facts in direct state/evidence. NEVER guess schemas, paths, or root causes. ALWAYS use relative paths for file operations. Reuse existing code, tools, and patterns before creating new ones.
...
2. Grounding: Inspect actual files and context before acting or drawing conclusions. ALWAYS use relative paths for file operations.
...
2. Paths: ALWAYS use relative paths (e.g., `path/to/file.py`) for all tool calls. NEVER construct `/worktrees/...` or absolute parent repo paths. NEVER modify files in parent repository paths.
