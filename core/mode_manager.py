...
BUILTIN_MODES = {
    ...
    "orchestrate": ModeDefinition(
        ...
        prompt=(
            ...
            "independently hit the same framework/harness quirks (async workers, mocking "
            "read-only properties, truthy query_one, etc.), that debugging cost is paid once "
            ...
            "7. Define reusable project subagents only when a role is genuinely reused across "
...
class ModeManager:
    ...
    def get_mode(self, key: str, project_dir: Optional[str] = None) -> ModeDefinition:
        ...
        # Fallback to act if not found
        return self.modes.get("act", BUILTIN_MODES["act"])
