...
def format_subagents_markdown(roles: List[Any], max_concurrent: int = 5) -> str:
    ...
    header = (
        ...
        "- self-contained: subagent has ZERO main chat history. `prompt` MUST be self-contained with relative paths (NEVER absolute paths), acceptance criteria, and expected output format.\n"
        "- title: noun phrase in English, 3-5 words (e.g. 'Auth token refactor'), not verbs.\n"
        f"- concurrency: ≤{max_concurrent} parallel. Hit limit → wait for completion notifications before spawning more; NEVER poll list.\n"
