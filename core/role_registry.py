...
BUILTIN_ROLES: Dict[str, AgentRole] = {
    "worker": AgentRole(
        ...
        prompt=(
            ...
            "3. Minimal Diffs & YAGNI: Only change what is strictly necessary. Do not add unsolicited refactorings, abstractions, or boilerplate comments.\n"
            "4. Task Tracking: For multi-step work, use `update_plan` and mark steps completed immediately after verification.\n"
            "5. Evidence-Based Verification (Iron Law): Run test/lint/build commands after edits. Never claim completion without fresh exit code 0 evidence.\n"
            "6. Regression Tests: For bug fixes, verify that a regression test fails before the fix and passes after.\n"
            "7. Git Safety: NEVER execute git commits, merges, or pushes unless explicitly asked."
        ),
        scope="any",
        source="builtin",
    ),
    ...
    "orchestrator": AgentRole(
        ...
        prompt=(
            ...
            "5. Worktree Isolation: Parallel writing subagents require unique `branch` names (worktree isolation). Use current branch only for read-only tasks or single workers.\n\n"
