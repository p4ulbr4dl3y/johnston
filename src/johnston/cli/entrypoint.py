...
from johnston.cli.commands.mcp_cmd import print_mcp
from johnston.cli.commands.provider_cmd import print_models
from johnston.core.infrastructure.platform.logging_setup import setup_logging
...
def get_version() -> str:
    ...
    cli_mod = sys.modules.get("johnston.cli.main") or sys.modules.get("johnston.cli") or sys.modules.get("cli")
...
def _dispatch_roles(args: Any = None) -> int:
    from johnston.cli.commands.roles_cmd import run_roles
    return run_roles(args)


def _dispatch_skills(args: Any = None) -> int:
    from johnston.cli.commands.skills_cmd import run_skills
    return run_skills(args)


def _dispatch_rules(args: Any = None) -> int:
    from johnston.cli.commands.rules_cmd import run_rules
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.subcommand == "config":
        from johnston.cli.commands.config_cmd import run_config

        code = run_config(args)
        sys.exit(code)
    elif args.subcommand == "provider":
        from johnston.cli.commands.provider_cmd import run_provider

        code = run_provider(args)
        sys.exit(code if code is not None else 0)
    elif args.subcommand == "mcp":
        from johnston.cli.commands.mcp_cmd import run_mcp
        ...
        from johnston.cli.commands.session_cmd import run_session

        code = run_session(args)
        sys.exit(code if code is not None else 0)
    elif args.subcommand == "doctor":
        from johnston.cli.commands.doctor_cmd import run_doctor
