...
from johnston.core.infrastructure.runtime.xml_utils import escape_xml, escape_xml_attr
...
def format_subagents_markdown(roles: List[Any], max_concurrent: int = 5) -> str:
    ...
    header = (
        "Delegate bounded tasks via `spawn_subagent(title=\"...\", task=\"...\", role=\"...\")` (background; auto-notify on completion).\n"
        "Rules:\n"
        ...
        "- limits: subagents cannot call `spawn_subagent`/`message_subagent`/`kill`/`ask_user`, cannot run background processes, cannot ask the user. Decisions needing the user go in the subagent's report.\n"
