...
- `subagent`: Usable only as `role` in `spawn_subagent`.
...
Subagents are spawned via `spawn_subagent(title="...", task="...", role="<role_key>")`:
...
- **Non-Interactive Exclusions**: `spawn_subagent`, `message_subagent`, `kill`, `ask_user`, and `shell(wait_seconds=...)` are automatically disabled in all non-interactive contexts (subagent roles and headless mode).