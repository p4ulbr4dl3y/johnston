...
class SubagentsListScreen(ModalScreen[None]):
    """Модальный экран со списком субагентов"""
    BINDINGS = [
        ("escape", "close", "Close Manager"),
        ("k", "kill_task", "Kill Subagent"),
    ]

    def compose(self) -> ComposeResult:
        with Vertical(id="modal-dialog"):
            yield Markdown("### **Subagents Manager**")
            yield OptionList(id="tasks-option-list")
            yield Label("Enter: View Output | K: Kill Subagent | Escape: Close", id="modal-hint")
    ...
    def _get_subagents(self):
        ...
        return [t for t in tasks if getattr(t, "task_id", "").startswith("subagent-")]
    ...
    def update_subagents_list(self) -> None:
        ...
        if current_highlighted is not None and current_highlighted < len(subagents):
            opt_list.highlighted = current_highlighted

    def on_option_list_option_selected(self, event: OptionList.OptionSelected) -> None:
        subagents = self._get_subagents()
        if event.option_index is not None and event.option_index < len(subagents):
            task = subagents[event.option_index]
            self.app.push_screen(TaskConsoleScreen(task))
    ...
    async def action_kill_task(self) -> None:
        ...
        opt_list = self.query_one("#tasks-option-list", OptionList)
        idx = opt_list.highlighted
        if idx is not None and idx < len(subagents):
            task = subagents[idx]
            if task.is_running:
                await task.kill()
                self.app.notify(f"Subagent {task.task_id} terminated.")
                self.update_subagents_list()
            else:
                self.app.notify("Subagent is already finished.", severity="warning")
