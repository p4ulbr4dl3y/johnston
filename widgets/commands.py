...
class DemoCommand(BaseCommand):
    name = "/demo"
    aliases = ["/test_tools", "/tools_demo"]
    description = "Run a live sequence of tool calls to test UI responsiveness"

    async def execute(self, app) -> None:
        chat_view = getattr(app, "chat_view", None) or app.query_one(ChatView)
        if not chat_view:
            return

        asyncio.create_task(self._run_demo(app, chat_view))

    async def _run_demo(self, app, chat_view) -> None:
        try:
            # 1. Thinking
            think = await chat_view.add_thinking_widget("Testing UI responsiveness across tools & Markdown...")
            await asyncio.sleep(0.3)
            think.finish_thinking(0.3, "Ready to execute tool and Markdown batch.")

            # 2. Markdown Message 1: Intro with formatting & list
            bot_msg1 = await chat_view.add_bot_message()
            intro_chunks = [
                "### 🚀 Starting Responsiveness Demo\n\n",
                "Testing **live stream**, *Markdown formatting*, and interleaved tool invocations:\n",
                "- ⏱️ **Zero-lag** event loop execution\n",
                "- 📦 **Multi-tool** pipeline (Plan, Read, Shell, Edit)\n",
                "- 📊 **Complex Markdown** tables and syntax blocks\n\n",
                "Now executing first tool...",
            ]
            for c in intro_chunks:
                bot_msg1.append_stream_content(c)
                await asyncio.sleep(0.06)
            await bot_msg1.finalize_stream()
            await asyncio.sleep(0.2)

            # 3. Tool 1: Plan update
            plan_widget = await chat_view.add_tool_call(
                "update_plan",
                "Plan",
                args={
                    "plan": {
                        "entries": [
                            {"step": "Render rich Markdown intro", "status": "completed"},
                            {"step": "Read pyproject.toml", "status": "in_progress"},
                            {"step": "Render syntax highlighted code block", "status": "pending"},
                            {"step": "Execute shell echo", "status": "pending"},
                            {"step": "Render Markdown table & diff", "status": "pending"},
                        ]
                    }
                },
            )
            await asyncio.sleep(0.3)
            plan_widget.set_result("Plan updated: step 1 done", status="done")

            # 4. Tool 2: Read
            read_widget = await chat_view.add_tool_call(
                "read", "pyproject.toml", args={"path": "pyproject.toml"}
            )
            await asyncio.sleep(0.2)
            from tools.registry import execute_tool

            res = await execute_tool("read", {"path": "pyproject.toml", "end_line": 15})
            read_widget.set_result(res.content if hasattr(res, "content") else str(res), status="done")

            # 5. Markdown Message 2: Code block with Python syntax highlighting
            bot_msg2 = await chat_view.add_bot_message()
            code_chunks = [
                "Here is an example python code block rendered in Markdown:\n\n",
                "```python\n",
                "import asyncio\n",
                "import time\n\n",
                "async def non_blocking_worker(items: list[str]) -> int:\n",
                "    \"\"\"Simulate fast async pipeline processing without UI stalls.\"\"\"\n",
                "    count = 0\n",
                "    for item in items:\n",
            ...
            for c in code_chunks:
                bot_msg2.append_stream_content(c)
                await asyncio.sleep(0.05)
            await bot_msg2.finalize_stream()
            await asyncio.sleep(0.2)

            # 6. Tool 3: Shell
            shell_widget = await chat_view.add_tool_call(
                "shell",
                "echo 'Johnston UI responsiveness check: PASS'",
                args={"command": "echo 'Johnston UI responsiveness check: PASS'"},
            )
            await asyncio.sleep(0.2)
            res_shell = await execute_tool("shell", {"command": "echo 'Johnston UI responsiveness check: PASS'"})
            shell_widget.set_result(
                res_shell.content if hasattr(res_shell, "content") else str(res_shell), status="done"
            )

            # 7. Tool 4: Edit diff
            edit_widget = await chat_view.add_tool_call(
                "edit", "demo.py", args={"path": "demo.py", "old_str": "val = 1", "new_str": "val = 2"}
            )
            await asyncio.sleep(0.2)
            synthetic_diff = "--- a/demo.py\n+++ b/demo.py\n@@ -1,3 +1,3 @@\n-val = 1\n+val = 2\n"
            edit_widget.set_result(synthetic_diff, status="done")

            # 8. Markdown Message 3: Table and stress markdown
            bot_msg3 = await chat_view.add_bot_message()
            table_chunks = [
                "### 📊 Benchmark Summary\n\n",
                "| Component | Status | Latency | Render Mode |\n",
                "| :--- | :---: | :---: | :--- |\n",
                "| `ToolCallWidget` | ✅ OK | < 10ms | Native TCSS |\n",
                "| `MarkdownParser` | ✅ OK | < 15ms | Async stream |\n",
                "| `DiffRenderer`   | ✅ OK | < 12ms | Split stream |\n\n",
                "> **Result:** All Markdown blocks & tool chips rendered smoothly with zero event loop blockages! 🎉\n",
            ]
            for c in table_chunks:
                bot_msg3.append_stream_content(c)
                await asyncio.sleep(0.05)
            await bot_msg3.finalize_stream()

        except Exception as e:
            if hasattr(app, "notify"):
                app.notify(f"Demo failed: {e}", severity="error")
