    ...
    def update_status(
        ...
        if is_compact:
            ...
            if is_connected and clean_model and clean_model != "[Select model: /models]":
                ...
                row1_left_parts.append(f"[{t_secondary}]{disp_model}[/]")
            ...
            if branch_disp and diff_text and width >= 50:
                row2_left_parts.append(f"[{t_primary}]{branch_disp}[/] [{t_secondary}]({diff_text})[/]")
            elif branch_disp:
                row2_left_parts.append(f"[{t_primary}]{branch_disp}[/]")
            elif diff_text:
            ...
            if subagents_active > 0:
                task_parts.append(f"[{t_secondary}]{subagents_active}a[/]")
            if active_bg_tasks > 0:
                task_parts.append(f"[{t_secondary}]{active_bg_tasks}s[/]")
            if mcp_total > 0:
                task_parts.append(f"[{t_secondary}]{mcp_active}mcp[/]")
            row2_right = f"[{t_secondary}]⚡[/] {sep_compact.join(task_parts)}" if task_parts else ""
            ...
            if is_connected and bool(model_name):
                ...
                row1_right_parts = [
                    f"[{t_subtle}][{bar_str}][/] [{t_secondary}]{pct:.0f}% ({used_formatted}/{context_window})[/]",
            ...
            if subagents_active > 0:
                service_parts.append(
                    f"[{t_secondary}]{subagents_active} agent[/]" if subagents_active == 1 else f"[{t_secondary}]{subagents_active} agents[/]"
                )
            if active_bg_tasks > 0:
                service_parts.append(f"[{t_secondary}]{active_bg_tasks} shell[/]")
            if mcp_total > 0:
                mcp_str = f"{mcp_active} MCP" if mcp_active == mcp_total else f"{mcp_active}/{mcp_total} MCP"
                service_parts.append(f"[{t_secondary}]{mcp_str}[/]")

            if service_parts:
                row2_right = f"[{t_secondary}]⚡[/] {sep.join(service_parts)}"
            else:
                row2_right = ""
