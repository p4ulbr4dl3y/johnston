...
from johnston_core.application.session.stream_agent import configure_subagent_agent
from johnston_core.application.session.stream_runner import _safe_save, run_subagent_stream_bg
...
async def send_subagent_followup(
    ...
    if session.async_task and hasattr(session.async_task, "done") and not session.async_task.done():
        ...
        from johnston_core.infrastructure.runtime.subagent_tracker import _mark_subagent_running
