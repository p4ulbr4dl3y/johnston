...
from johnston_core.application.session.stream import record_session_step, sync_session_metrics
...
async def generate_ai_response(
    ...
    *,
    ...
    driver = (
        ...
        if ChatStreamDriver is not None
        else None
    )
    ...
    try:
        ...
        await _handle_interruption(
            ...
            session,
            canvas,
            driver.thinking_handle if driver is not None else None,
            driver.bot_handle if driver is not None else None,
            start_time=start_time,
            tool_handles=driver.tool_handles if driver is not None else None,
        )
        ...
        if driver is not None and hasattr(driver, "cleanup_unfinalized_tools"):
            try:
        ...
        if driver is not None and hasattr(driver, "cleanup_unfinalized_tools"):
            try:
                ...
                pass
        if driver is not None and driver.thinking_handle is not None and getattr(driver.thinking_handle, "is_thinking", False):
            try:
                duration = time.time() - start_time
                driver.thinking_handle.finish_thinking(duration)
            except Exception:  # noqa: BLE001
                pass
        if driver is not None and driver.bot_handle is not None and not getattr(driver.bot_handle, "content", "").strip():
            try:
