...
from johnston_core.application.generation.git_info import (
...
from johnston_core.application.generation.project_rules import (
...
from johnston_core.infrastructure.runtime.xml_utils import escape_xml
...
class PromptBuilder:
    ...
    def __init__(
        ...
        if sandbox_enabled is not None:
            ...
            from johnston_core.infrastructure.config.config_helpers import load_sandbox_config
    ...
    def _base_sys_prompt(self) -> str:
        ...
        if "{compaction_ratio}" in sys_prompt:
            try:
                ...
                from johnston_core.domain.defaults.config import DEFAULT_COMPACTION_THRESHOLD_RATIO
    ...
    def _stable_core_cached(self, rules_snippet, mcp_snippet, skills_snippet, subagents_snippet) -> Optional[str]:
        ...
        """
        from johnston_core.role_registry import BUILTIN_ROLES, RoleRegistry
    ...
    def _assemble_stable_core(self, rules_snippet, mcp_snippet, skills_snippet, subagents_snippet, role_def=None) -> str:
        ...
        if getattr(role_def, "prompt", None) and not self.is_subagent and "<role" not in base:
            from johnston_core.roles.prompt import format_role_prompt
