...
from johnston_core.infrastructure.platform.logging_setup import adopt_task_exception
...
_pkg_css = Path(__file__).resolve().parents[1] / "app.tcss"
_CSS_PATH = _pkg_css if _pkg_css.exists() else Path(__file__).resolve().parents[2] / "app.tcss"
...
class JohnstonApp(LifecycleMixin, MessageFlowMixin, SessionPersistenceMixin, ActionsMixin, App):
    ...
    def __init__(
        ...
        from johnston_tui.app.startup import (
            apply_startup_flags,
            build_agent,
    ...
    def copy_to_clipboard(self, text: str, notify: bool = True) -> None:
        ...
        try:
            ...
            pass
        from johnston_core.infrastructure.platform.platform_utils import copy_to_os_clipboard_async
