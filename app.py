...
class TUIChatApp(App):
    """Минималистичный TUI чат с ИИ-агентом"""
    ...
    def compose(self) -> ComposeResult:
        with Vertical(id="app-container"):
            ...
            yield Input(placeholder="Сообщение...", id="message-input")

    async def on_mount(self) -> None:
        """Приветственное сообщение при запуске"""
        chat_view = self.query_one(ChatView)
        welcome_bubble = await chat_view.add_bot_message(persona_name="ИИ")
        welcome_bubble.content = (
            "Привет! Задай любой вопрос.\n"
            "Нажми `Enter` для отправки, `Esc` для выхода."
        )
    ...
    async def generate_ai_response(self, user_text: str) -> None:
        ...
        bot_bubble = await chat_view.add_bot_message(persona_name="ИИ")
